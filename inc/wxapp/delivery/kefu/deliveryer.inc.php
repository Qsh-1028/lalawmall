...
mload()->model('plugin');
pload()->model('kefu');
$_W["kefu"]["user"] = $kefu = array("role" => "deliveryer", "deliveryer_id" => $_W["deliveryer"]["id"], "token" => $_W["deliveryer"]["token"], "nickname" => $_W["deliveryer"]["nickname"], "avatar" => tomedia($_W["deliveryer"]["avatar"]));
$ta = trim($_GPC["ta"]) ? trim($_GPC["ta"]) : "chat";
if ($ta == "chat") {
    $relation = trim($_GPC["relation"]) ? trim($_GPC["relation"]) : "member2deliveryer";
    if (empty($relation)) {
    ...
    $result = array("chatlog" => $chatlog, "chat" => $chat, "kefu" => $kefu, "fans" => $fans, "reply" => kefu_get_fastreply($kefu, $relation));
    ...
    if ($ta == "addchat") {
        ...
        if ($ta == "addreply") {
            ...
            if ($ta == "index") {
                ...
                $result = array("chats" => $chats, "deliveryer" => $deliveryer);
                imessage(error(0, $result), "", "ajax");
            } else {
                if ($ta == "order") {
                    $chatid = intval($_GPC["chatid"]);
