...
if($ta == 'list') {
	...
	if(!empty($orders)) {
		...
		foreach($orders as $key => &$row) {
			...
			if($row['status'] == 2 && $config_errander['delivery_timeout_limit'] > 0) {
				$delivery_overtime = $row['delivery_assign_time'] + $config_errander['delivery_timeout_limit'] * 60;
				if(TIMESTAMP > $delivery_overtime) {
	...
	$result = array(
		...
		'config' => array(
			'auto_refresh' => $config_errander['auto_refresh']
		)
	);
...
elseif($ta == 'detail') {
	...
	if($order['status'] == 2 && $config_errander['delivery_timeout_limit'] > 0) {
