                ...
                if ($ta == "suggestion") {
                    ...
                    if ($ta == "suggestion_google") {
                        ...
                        if ($ta == "place_around_google") {
                            ...
                            $result = @json_decode($result["content"], true);
                            if ($result["status"] != "OK") {
                                $error_messages = array("ZERO_RESULTS" => "未能搜索到附近的位置", "OVER_QUERY_LIMIT " => "请求已超过配额", "REQUEST_DENIED" => "请求被拒绝，通常是因为缺少key参数", "INVALID_REQUEST" => "缺少必需的查询参数location或者radius", "UNKNOWN_ERROR" => "服务器端错误");
