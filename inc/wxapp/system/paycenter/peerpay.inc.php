...
if ($ta == 'paylist') {
	...
	$peerpay['data']['logo'] = tomedia($peerpay['data']['logo']);
	$result = array('member' => $member, 'peerpay' => $peerpay, 'payinfos' => $payinfos, 'differ' => $differ, 'percent' => $percent, 'page_title' => empty($member['nickname']) ? '我的代付' : $member['nickname'] . '的代付', 'is_same_person' => $_W['member']['uid'] == $member['uid'] ? 1 : 0);
	$title = '亲爱哒，帮我付一下呗~';
...
if ($ta == 'payment') {
	...
	$member['avatar'] = tomedia($member['avatar']);
	$result = array('member' => $member, 'peerpay' => $peerpay, 'note' => $note, 'page_title' => empty($member['nickname']) ? '我的代付' : $member['nickname'] . '的代付');
	imessage(error(0, $result), '', 'ajax');
}
