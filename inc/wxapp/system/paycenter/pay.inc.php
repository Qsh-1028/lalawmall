...
$params = array("module" => "we7_wmall", "ordersn" => $record["order_sn"], "tid" => $record["order_sn"], "user" => $_W["member"]["openid_wxapp"], "fee" => $record["fee"], "title" => $title, "order_type" => $type, "sid" => $order["sid"], "title" => urldecode($title), "dollarrate" => $_W["dollarrate"], "global_fee" => round($record["fee"] * $_W["dollarrate"], 2));
$log = pdo_get("core_paylog", array("uniacid" => $_W["uniacid"], "module" => $params["module"], "tid" => $params["tid"]));
if (empty($log)) {
    $log = array("uniacid" => $_W["uniacid"], "acid" => $_W["acid"], "openid" => $params["user"], "module" => $params["module"], "uniontid" => date("YmdHis") . random(14, 1), "tid" => $params["tid"], "fee" => $params["fee"], "card_fee" => $params["fee"], "status" => "0", "is_usecard" => "0", "tag" => array());
    if (!empty($_W["dollarrate"])) {
        $log["tag"]["dollarrate"] = $_W["dollarrate"];
    }
    $log["tag"] = iserializer($log["tag"]);
    ...
    if ($log["card_fee"] != $params["fee"]) {
        $updatelog = array("fee" => $params["fee"], "card_fee" => $params["fee"]);
    }
    if (!empty($_W["dollarrate"])) {
        $log["tag"]["dollarrate"] = $_W["dollarrate"];
        $log["tag"] = iserializer($log["tag"]);
    ...
    if (!empty($updatelog)) {
        pdo_update("core_paylog", $updatelog, array("plid" => $log["plid"]));
        $log["fee"] = $params["fee"];
...
if ($_GPC["type"]) {
    ...
    if (is_h5app()) {
        ...
        $payinfo = array("mallName" => $_config["mall"]["title"], "money" => round($log["card_fee"] * $_W["dollarrate"], 2), "ordersn" => $log["uniontid"], "desc" => $title, "attach" => (string) $_W["uniacid"] . ":h5app", "rsa_type" => $config_alipay["rsa_type"] ? $config_alipay["rsa_type"] : "RSA");
        $_W["h5appinfo"] = array("payinfo" => $payinfo);
    } else {
        if (is_qianfan()) {
            $type_pay = get_plugin_config("qianfanApp.type_pay_id");
            $payinfo = array("type" => $type_pay, "item" => json_encode(array(array("title" => $title, "cover" => "", "num" => 1, "gold_cost" => 0, "cash_cost" => $_W["member"]["uid"] == "112813212" ? 0.01 : round($log["card_fee"] * $_W["dollarrate"], 2)))), "address" => json_encode(array("name" => $order["username"], "mobile" => $order["mobile"], "address" => $order["address"])), "send_type" => 0, "allow_pay_type" => 14, "uniontid" => $log["uniontid"], "tid" => $params["tid"], "url_pay" => $routers[$type]["url_pay"], "url_detail" => $routers[$type]["url_detail"]);
            $_W["qianfan"] = array("payinfo" => $payinfo);
        } else {
            if (is_majia()) {
                mload()->model('plugin');
                pload()->model('majiaApp');
                $payinfo = array("uniontid" => $log["uniontid"], "orderNum" => $log["uniontid"], "trade_no" => $log["uniontid"], "title" => $title, "amount" => round($log["card_fee"] * $_W["dollarrate"], 2), "money" => round($log["card_fee"] * $_W["dollarrate"], 2), "user_id" => $_W["member"]["uid_majia"], "des" => "", "remark" => "", "type" => (string) $_W["we7_wmall"]["config"]["mall"]["title"] . "订单支付", "url_pay" => $routers[$type]["url_pay"], "url_detail" => $routers[$type]["url_detail"]);
...
if ($pay_type && !$_GPC["type"] && in_array($pay_type, array_keys($payment))) {
    ...
    if ($pay_type == "credit") {
        if ($_W["member"]["credit2"] < $params["fee"]) {
            imessage(error(-1000, "余额不足以支付, 需要 " . $params["fee"] . ", 当前 " . $_W["member"]["credit2"] . " " . $_W["Lang"]["dollarSignCn"]), "", "ajax");
        }
        $fee = floatval($params["fee"]);
        $result = member_credit_update($_W["member"]["uid"], "credit2", 0 - $fee, array($_W["member"]["uid"], "消费余额:" . $fee . $_W["Lang"]["dollarSignCn"]));
        if (is_error($result)) {
