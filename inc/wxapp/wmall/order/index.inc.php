...
if ($ta == "list") {
    ...
    if (!empty($orders)) {
        ...
        foreach ($orders as &$da) {
            ...
            if ($da["data"]["yinsihao_status"] == 1 && (empty($yinsihao) || empty($yinsihao["status"]))) {
                $da["data"]["yinsihao_status"] = 0;
            }
        }
    ...
    if ($ta == "detail") {
        ...
        $order_status = order_status();
        $store = store_fetch($order["sid"], array("id", "title", "telephone", "pack_price", "logo", "delivery_price", "address", "location_x", "location_y", "data", "kefu_status"));
        ...
        $order["kefu"] = array("deliveryer_status" => 0, "store_status" => 0);
        $config_kefu = get_plugin_config("kefu.system");
        if (check_plugin_perm('managerApp') && $config_kefu["status"] == 1 && $config_kefu["store_status"] == 1 && $store["kefu_status"] != 3) {
            $order["kefu"]["store_status"] = 1;
        }
        if (check_plugin_perm('deliveryerApp') && $config_kefu["status"] == 1 && $config_kefu["deliveryer_status"] == 1 && $deliveryer["kefu_status"] != 3) {
            $order["kefu"]["deliveryer_status"] = 1;
        }
        ...
        if ($ta == "new_detail") {
            ...
            $deliveryer = pdo_get("tiny_wmall_deliveryer", array("uniacid" => $_W["uniacid"], "id" => $order["deliveryer_id"]), array("id", "token", "title", "avatar", "mobile", "location_x", "location_y", "kefu_status"));
            if ($order["data"]["yinsihao_status"] == 1) {
                $config_yinsihao = get_plugin_config("yinsihao.basic");
                if ($config_yinsihao["member_call_deliveryer_status"] == 1) {
                    $deliveryer["mobile"] = substr_replace($deliveryer["mobile"], "****", 3, 4);
            ...
            if (check_plugin_perm('deliveryerApp') && $config_kefu["status"] == 1 && $config_kefu["deliveryer_status"] == 1) {
                $order["kefu"]["deliveryer_status"] = 1;
            }
                                                ...
                                                if ($ta == "zhunshibao") {
                                                    ...
                                                    if (!empty($store["data"]["zhunshibao"]["rule"])) {
                                                        ...
                                                        foreach ($store["data"]["zhunshibao"]["rule"] as $val) {
                                                            ...
                                                            $zhunshibao_tips[] = "晚于" . $compensate_time_cn . "送达，赔付" . $compensate_fee . $_W["Lang"]["dollarSignCn"];
                                                        }
                                                    }
