...
if ($ta == "index") {
    ...
    $result = array("store" => $store, "coupon" => $coupons, "cart" => cart_data_init($sid), "template" => $template, "template_page" => $template_page, "lazyload_goods" => $_config_mall["lazyload_goods"], "config_mall" => $_config_mall, "member_kabao" => $_W["member"]["kabao"]);
    if ($template_page == 1) {
            ...
            if ($ta == "truncate") {
                ...
                if ($ta == "cart") {
                    ...
                    if ($ta == "shopPage") {
                        ...
                        $result = array("goods" => $goods, "store" => $store, "cart" => cart_data_init($sid), "member_kabao" => $_W["member"]["kabao"]);
                        imessage(error(0, $result), "", "ajax");
                    } else {
                        if ($ta == "pindan") {
