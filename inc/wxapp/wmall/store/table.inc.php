...
if ($ta == "index") {
    ...
    $result = array("store" => $store, "coupon" => $coupons, "category" => $categorys, "cart" => cart_data_init($sid), "template" => $template, "template_page" => $template_page, "share" => $_share, "table" => $table);
                    ...
                    if ($ta == "detail") {
                        ...
                        if ($ta == "create") {
                            ...
                            if ($ta == "submit") {
                                ...
                                $person_num = intval($condition["person_num"]);
                                $box_price = round($box_price_tangshi * $person_num, 2);
                                $order = array("uniacid" => $_W["uniacid"], "acid" => $_W["acid"], "sid" => $sid, "uid" => $_W["member"]["uid"], "ordersn" => date("YmdHis") . random(6, true), "serial_sn" => store_order_serial_sn($sid), "code" => random(4, true), "order_type" => 3, "openid" => $_W["openid"], "mobile" => "", "username" => trim($condition["username"]), "person_num" => $person_num, "table_id" => $table_id, "sex" => "", "address" => "", "location_x" => "", "location_y" => "", "delivery_day" => "", "delivery_time" => "", "delivery_fee" => 0, "pack_fee" => 0, "pay_type" => "", "num" => $cart["num"], "price" => $cart["price"], "serve_fee" => $serve_fee, "box_price" => $box_price, "total_fee" => $cart["price"] + $serve_fee + $box_price, "discount_fee" => $activityed["total"], "store_discount_fee" => $activityed["store_discount_fee"], "plateform_discount_fee" => $activityed["plateform_discount_fee"], "final_fee" => $cart["price"] + $serve_fee + $box_price - $activityed["total"], "status" => 1, "is_comment" => 0, "invoice" => $invoice, "addtime" => TIMESTAMP, "data" => array("cart" => $cart["original_data"]), "note" => trim($condition["note"]), "storebd_id" => intval($store["storebd_id"]));
