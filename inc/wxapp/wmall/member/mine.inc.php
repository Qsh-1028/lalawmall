...
if ($_config_wxapp["diy"]["use_diy_member"] != 1) {
    ...
    if (check_plugin_perm('kefu') && get_plugin_config('kefu.system.status') == 1) {
        $kefu["status"] = 1;
        $kefu["notread"] = intval(pdo_fetchcolumn("select sum(notread) from " . tablename("tiny_wmall_kefu_chat") . " where uniacid = :uniacid and fansopenid = :fansopenid", array(":uniacid" => $_W["uniacid"], ":fansopenid" => $_W["member"]["token"])));
    }
    $result = array("config" => $_W["we7_wmall"]["config"], "redpacket_nums" => $redpacket_nums, "coupon_nums" => $coupon_nums, "credit2" => floatval($user["credit2"]), "credit1" => floatval($user["credit1"]), "user" => $user, "deliveryCard_status" => $deliveryCard_status, "deliveryCard_setmeal_ok" => $deliveryCard_setmeal_ok, "spread" => $spread, "ordergrant" => $ordergrant, "slides" => $slides, "cover" => $cover, "mealRedpacket" => $mealRedpacket, "has_gohome" => $has_gohome, "has_wheel" => $has_wheel, "has_svip" => $has_svip, "kefu" => $kefu);
} else {
