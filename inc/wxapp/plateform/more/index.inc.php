...
if ($ta == "index") {
    ...
    if ($_W["plateformer"]["role"] == "operator" && check_plugin_perm("kefu") && $config_kefu["status"] == 1 && $config_kefu["kefu_status"] == 1) {
        $urls["plugin"]["kefu"] = array("title" => "消息中心", "icon" => "icon-message", "iconColor" => "c-danger", "url" => "/pages/plugin/kefu/index", "checkperm" => "kefu", "notread" => intval(pdo_fetchcolumn("select sum(kefunotread) from " . tablename("tiny_wmall_kefu_chat") . " where uniacid = :uniacid and kefuopenid = :kefuopenid", array(":uniacid" => $_W["uniacid"], ":kefuopenid" => $_W["plateformer"]["token"]))));
    }
