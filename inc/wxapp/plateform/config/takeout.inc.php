...
if ($ta == 'order') {
	...
	if ($_W['ispost']) {
		...
		$grant_credit = array(
			'credit1' => array('status' => intval($_GPC['credit1']['status']), 'grant_type' => intval($_GPC['credit1']['grant_type']))
			);
		...
		$order = array(
			'notify_rule_clerk'                     => array('notify_delay' => intval($_GPC['clerk']['notify_delay']), 'notify_frequency' => intval($_GPC['clerk']['notify_frequency']), 'notify_total' => intval($_GPC['clerk']['notify_total']), 'notify_phonecall_time' => intval($_GPC['clerk']['notify_phonecall_time'])),
			'notify_rule_deliveryer'                => array('notify_delay' => intval($_GPC['deliveryer']['notify_delay']), 'notify_frequency' => intval($_GPC['deliveryer']['notify_frequency']), 'notify_total' => intval($_GPC['deliveryer']['notify_total']), 'notify_phonecall_time' => intval($_GPC['deliveryer']['notify_phonecall_time'])),
			'pay_time_limit'                        => intval($_GPC['pay_time_limit']),
			'pay_time_notice'                       => intval($_GPC['pay_time_notice']),
			'handle_time_limit'                     => intval($_GPC['handle_time_limit']),
			'auto_success_hours'                    => intval($_GPC['auto_success_hours']),
			'tangshi_auto_success_hours'            => intval($_GPC['tangshi_auto_success_hours']),
			'deliveryer_collect_time_limit'         => intval($_GPC['deliveryer_collect_time_limit']),
			'cancel_after_handle'                   => intval($_GPC['cancel_after_handle']),
			'auto_refund_cancel_order'              => intval($_GPC['auto_refund_cancel_order']),
			...
			'store_sailed_type'                     => trim($_GPC['store_sailed_type']),
			...
			'delivery_status_3'                     => $delivery_status_3,
			'delivery_status_4'                     => $delivery_status_4,
			'delivery_status_7'                     => $delivery_status_7,
			'show_acceptaddress_when_firstdelivery' => intval($_GPC['show_acceptaddress_when_firstdelivery']),
			'customer_delete_order'                 => intval($_GPC['customer_delete_order']),
			'deliverynoassign_sort_type'            => trim($_GPC['deliverynoassign_sort_type']),
			'check_member_drag_address'             => intval($_GPC['check_member_drag_address']),
			'use_default_accept_address'            => intval($_GPC['use_default_accept_address']),
			'audit_accept_address'                  => intval($_GPC['audit_accept_address'])
			);
