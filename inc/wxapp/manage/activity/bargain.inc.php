	...
	if($_W['ispost']) {
		...
		if(!empty($bargain_goods)) {
			foreach($bargain_goods as $value) {
				...
				if (empty($temp) || ($temp['bargain_id'] > 0 && $temp['bargain_id'] != $id)) {
					continue;
				}
				$row = array(
					'goods_id' => $value['id'],
					'discount_price' => floatval($value['discount_price']),
					'max_buy_limit' => intval($value['max_buy_limit']),
					'poi_user_type' => trim($value['poi_user_type']) == 'all' ? 'all' : 'new',
					'discount_total' => intval($value['discount_total']),
					'discount_available_total' => intval($value['discount_available_total']),
				);
				$goods[$value['id']] = $row;
			}
		}
		...
		$data = array(
			...
			'sid' => $sid,
			'title' => $title,
			'content' => trim($bargain['content']),
			'order_limit' => intval($bargain['order_limit']),
			'goods_limit' => intval($bargain['goods_limit']),
			'starttime' => intval($bargain['starttime']),
			'endtime' => intval($bargain['endtime']),
			'starthour' => str_replace(':', '', trim($bargain['starthour'])),
			'endhour' => str_replace(':', '', trim($bargain['endhour'])),
			'use_limit' => intval($bargain['use_limit']),
			'addtime' => TIMESTAMP,
			'total_updatetime' => strtotime(date('Y-m-d')) + 86400,
			'thumb' => trim($bargain['thumb']),
		);
		$activity = array(
			'uniacid' => $_W['uniacid'],
			'sid' => $sid,
...
elseif($ta == 'goods_list') {
	...
	$params[':cid'] = $cid;
	$condition .= ' and is_options = 0 and svip_status = 0 and kabao_status = 0';
