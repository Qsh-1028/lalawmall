...
if ($ta == 'recommend') {
	...
	if ($_W['ispost']) {
		...
		$update = array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'sid' => $sid, 'type' => $type, 'title' => $page_title . '幻灯片展示' . $day . '天', 'status' => 0, 'addtime' => TIMESTAMP, 'starttime' => TIMESTAMP, 'endtime' => TIMESTAMP, 'is_pay' => 0, 'order_sn' => date('YmdHis', time()) . random(6, true), 'final_fee' => $finalfee, 'pay_type' => $pay_type, 'days' => $day, 'data' => '');
		pdo_insert('tiny_wmall_advertise_trade', $update);
		$id = pdo_insertid();
		imessage(error(0, array('id' => $id, 'sid' => $sid)), '', 'ajax');
	}
	...
	$result = array(
		'advertise' => array('recommendHome' => $recommendHome, 'recommendOther' => $recommendOther),
		'amount'    => $amount
		);
	imessage(error(0, $result), '', 'ajax');
}
