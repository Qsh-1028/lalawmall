...
if ($ta == "list") {
    ...
    $result = array("orders" => $orders, "openid_wxapp_manager" => $_W["manager"]["openid_wxapp_manager"], "store" => array("data" => $_W["we7_wmall"]["store"]["data"], "cn" => $_W["we7_wmall"]["store"]["cn"]), "userAgreement" => get_config_text("manager:agreement"));
                                    ...
                                    if ($ta == "push_dada") {
                                        ...
                                        if ($ta == "push_uupaotui") {
                                            ...
                                            if ($_W["ispost"]) {
                                                ...
                                                if (!$push) {
                                                    $result = array("tips" => "UU跑腿需要配送费" . $status["need_paymoney"] . $_W["Lang"]["dollarSignCn"] . "，确定推送到UU跑腿吗？", "id" => $id);
                                                } else {
                                                    $result = "订单推送到UU跑腿成功";
                                                }
                                                imessage(error(0, $result), "", "ajax");
...
if ($ta == "push_shansong") {
    ...
    if ($_W["ispost"]) {
        ...
        if (!$push) {
            $result = array("tips" => "闪送配送需要配送费" . $status["amount"] . $_W["Lang"]["dollarSignCn"] . "，确定推送闪送吗？", "id" => $id);
        } else {
            $result = "订单推送到闪送成功";
        }
        imessage(error(0, $result), "", "ajax");
    ...
    if ($ta == "push_dianwoda") {
        ...
        if ($_W["ispost"]) {
            ...
            if (!$push) {
                $result = array("tips" => "点我达配送需要配送费" . $fee . $_W["Lang"]["dollarSignCn"] . "，确定推送点我达吗？", "id" => $id);
            } else {
                $result = "订单成功推送到点我达";
            }
            imessage(error(0, $result), "", "ajax");
