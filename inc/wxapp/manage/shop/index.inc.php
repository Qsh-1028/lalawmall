...
if ($ta == "index") {
    ...
    if (0 < $storebd_id) {
        mload()->model("plugin");
        pload()->model('storebd');
        $store["storebd_user"] = storebd_user_fetch($storebd_id);
    }
    ...
    $kefu = array("status" => 0, "notread" => 0);
    $config_kefu = get_plugin_config("kefu.system");
    if (check_plugin_perm('kefu') && $config_kefu["status"] == 1 && $config_kefu["store_status"] == 1) {
        $kefu["status"] = 1;
        $kefu["notread"] = intval(pdo_fetchcolumn("select sum(kefunotread) from " . tablename("tiny_wmall_kefu_chat") . " where uniacid = :uniacid and kefuopenid = :kefuopenid", array(":uniacid" => $_W["uniacid"], ":kefuopenid" => $_W["manager"]["token"])));
    }
    $result = array("store" => $store, "manager" => $_W["manager"], "stat" => $stat, "notice" => $notice, "ads" => $ads, "order_notice_num" => $order_notice_num, "kefu" => $kefu);
                        ...
                        if ($ta == "account") {
                            ...
                            if ($ta == "pill") {
                                ...
                                if ($ta == "slog") {
                                    ...
                                    if ($ta == "agreement") {
                                        $result = array("agreement" => get_config_text("agreement_settle"));
                                        imessage(error(0, $result), "", "ajax");
                                    }
                                }
                            }
