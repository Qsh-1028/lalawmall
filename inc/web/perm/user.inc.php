...
if ($op == "post") {
    ...
    if ($_W["ispost"]) {
        load()->model("user");
        $member = array();
        $insert = array("uniacid" => $_W["uniacid"], "roleid" => intval($_GPC["roleid"]), "status" => intval($_GPC["status"]), "realname" => trim($_GPC["realname"]), "mobile" => trim($_GPC["mobile"]), "nickname" => trim($_GPC["nickname"]), "avatar" => trim($_GPC["avatar"]), "perms" => implode(",", $_GPC["perms"]));
        if (0 < $insert["roleid"]) {
        ...
        if (empty($id)) {
            ...
            $uid = user_register($member, "admin");
            if (is_error($uid)) {
