...
if ($op == "uy") {
    $stores = pdo_getall("tiny_wmall_store", array("uniacid" => $_W["uniacid"]), array("id", "title", "title_uy"));
    foreach ($stores as $store) {
        $update = array("title_ch" => uych($store["title"], "ch"));
        $title_uy = uych($store["title"]);
        if (empty($store["title_uy"]) && !empty($title_uy)) {
            $update["title_uy"] = $title_uy;
        }
        p($update);
    }
    return NULL;
    ...
    if ($op == "batch") {
        ...
        if ($type == "is_in_business") {
            ...
            pdo_query('update ' . tablename('tiny_wmall_store') . " set " . $type . " = " . $value . " where uniacid = " . $_W["uniacid"] . " and status not in (2, 3, 4)");
        }
        imessage(error(0, ""), "", 'ajax');
    }
    if ($op == "copy") {
