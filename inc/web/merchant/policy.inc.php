...
if ($op == "list") {
    ...
    if (!empty($policys)) {
        foreach ($policys as &$value) {
            ...
            $value["starttime_cn"] = 0 < $value["starttime"] ? date("Y-m-d H:i", $value["starttime"]) : "未开始";
            $value["endtime_cn"] = 0 < $value["endtime"] ? date("Y-m-d H:i", $value["endtime"]) : "未结束";
    ...
    if ($op == "post") {
        ...
        if ($_W["ispost"]) {
            $insert = array("uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "title" => trim($_GPC["title"]), "addtime" => TIMESTAMP, "status" => 1);
            $policy = array("change_type" => trim($_GPC["change_type"]), "send_price" => floatval($_GPC["send_price"]), "delivery_price" => floatval($_GPC["delivery_price"]), "pre_km_fee" => floatval($_GPC["pre_km_fee"]));
        ...
        if ($op == "start") {
            ...
            if (!empty($store_ids)) {
                foreach ($store_ids as $sid) {
                    ...
                    $insert = array("uniacid" => $_W["uniacid"], "policyid" => $policy["id"], "sid" => $store["id"], "original" => iserializer($original), "changed" => iserializer($changed), "addtime" => TIMESTAMP);
                    pdo_insert('tiny_wmall_store_delivery_policy_log', $insert);
                }
            }
            $update = array("status" => 2, "starttime" => TIMESTAMP);
            ...
            if ($op == "end") {
                ...
                $update = array("status" => 3, "endtime" => TIMESTAMP);
                pdo_update('tiny_wmall_store_delivery_policy', $update, array("uniacid" => $_W["uniacid"], "id" => $policy["id"]));
