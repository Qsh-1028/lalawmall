...
if ($ta == "index") {
    ...
    if ($_W["ispost"]) {
        ...
        $activity = array("uniacid" => $_W["uniacid"], "sid" => $sid, "title" => "平台新用户立减" . $back . $_W["Lang"]["dollarSignCn"], "starttime" => $starttime, "endtime" => $endtime, "type" => "mallNewMember", "status" => 1, "data" => array("back" => $back, "plateform_charge" => 0, "store_charge" => $back));
        if (!empty($_W["ismanager"])) {
