...
if ($op == "setmeal") {
    ...
    if ($_W["ispost"]) {
        ...
        mlog(6005, intval($_GPC["uid"]), "套餐id：" . $_GPC["setmeal"] . "， 套餐开始-结束：" . $_GPC["setmeal_starttime"] . "-" . $_GPC["setmeal_endtime"] . "，每天可享受免减配送次数：" . $_GPC["free"] . "， 每单最多：" . $_GPC["deliveryfee"] . $_W["Lang"]["dollarSignCn"]);
...
if ($op == "info") {
    ...
    if ($_W["ispost"]) {
        $member = array("avatar" => trim($_GPC["avatar"]), "nickname" => trim($_GPC["nickname"]), "realname" => trim($_GPC["realname"]), "mobile" => trim($_GPC["mobile"]), "salt" => $member["salt"]);
