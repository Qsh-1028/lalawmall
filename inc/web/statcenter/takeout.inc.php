...
if( $op == "index" ) 
{
    ...
    if( $_W["isajax"] ) 
    {
        ...
        $chart = array( "stat" => $stat, "fields" => array( "total_success_order", "total_fee", "final_fee", "plateform_delivery_fee", "plateform_serve_fee", "plateform_deliveryer_fee", "plateform_discount_fee", "agent_serve_fee", "agent_discount_fee", "refund_fee", "extra_fee" ), "titles" => array( "有效订单量", "营业总额", "总入账", "平台配送费收入", "佣金收入", "配送员配送费支出", "平台补贴", "平台拥金", "代理补贴", "总退款", "附加费" ) );
        if( $perm_zhunshibao ) 
        {
    ...
    while( $starttime <= $i ) 
    {
        ...
        {
            $records[] = array( "stat_day" => $i, "total_success_order" => 0, "total_fee" => 0, "final_fee" => 0, "plateform_serve_fee" => 0, "plateform_deliveryer_fee" => 0, "plateform_delivery_fee" => 0, "plateform_discount_fee" => 0, "agent_serve_fee" => 0, "agent_discount_fee" => 0, "refund_fee" => 0, "extra_fee" => 0 );
        }
        else
        {
