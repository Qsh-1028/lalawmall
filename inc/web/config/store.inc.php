...
if ($op == "settle") {
    ...
    if ($op == "serve_fee") {
        ...
        if ($_W["ispost"]) {
            ...
            if ($form_type == "serve_fee_setting") {
                ...
                if ($serve_fee["fee_takeout"]["type"] == 2) {
                    ...
                    if ($serve_fee["fee_takeout"]["type"] == 3) {
                        $serve_fee["fee_takeout"]["level1"] = array("fee_start" => floatval($takeout_GPC["level1"]["fee_start"]), "fee" => floatval($takeout_GPC["level1"]["fee"]));
                        if (empty($serve_fee["fee_takeout"]["level1"]["fee_start"]) || empty($serve_fee["fee_takeout"]["level1"]["fee"])) {
                            imessage(error(-1, "请设置外卖单佣金计算方式的阶梯固定抽成阶梯一的订单起抽金额和抽成金额"), "", "ajax");
                        }
                        $serve_fee["fee_takeout"]["level2"] = array("fee_start" => floatval($takeout_GPC["level2"]["fee_start"]), "fee" => floatval($takeout_GPC["level2"]["fee"]));
                        if (empty($serve_fee["fee_takeout"]["level2"]["fee_start"]) || empty($serve_fee["fee_takeout"]["level2"]["fee"])) {
                ...
                if ($serve_fee["fee_selfDelivery"]["type"] == 2) {
                    ...
                    if ($serve_fee["fee_selfDelivery"]["type"] == 3) {
                        $serve_fee["fee_selfDelivery"]["level1"] = array("fee_start" => floatval($selfDelivery_GPC["level1"]["fee_start"]), "fee" => floatval($selfDelivery_GPC["level1"]["fee"]));
                        if (empty($serve_fee["fee_selfDelivery"]["level1"]["fee_start"]) || empty($serve_fee["fee_selfDelivery"]["level1"]["fee"])) {
                            imessage(error(-1, "请设置自提单佣金计算方式的阶梯固定抽成阶梯一的订单起抽金额和抽成金额"), "", "ajax");
                        }
                        $serve_fee["fee_selfDelivery"]["level2"] = array("fee_start" => floatval($selfDelivery_GPC["level2"]["fee_start"]), "fee" => floatval($selfDelivery_GPC["level2"]["fee"]));
                        if (empty($serve_fee["fee_selfDelivery"]["level2"]["fee_start"]) || empty($serve_fee["fee_selfDelivery"]["level2"]["fee"])) {
                ...
                if ($serve_fee["fee_instore"]["type"] == 2) {
                    ...
                    if ($serve_fee["fee_instore"]["type"] == 3) {
                        $serve_fee["fee_instore"]["level1"] = array("fee_start" => floatval($instore_GPC["level1"]["fee_start"]), "fee" => floatval($instore_GPC["level1"]["fee"]));
                        if (empty($serve_fee["fee_instore"]["level1"]["fee_start"]) || empty($serve_fee["fee_instore"]["level1"]["fee"])) {
                            imessage(error(-1, "请设置店内单佣金计算方式的阶梯固定抽成阶梯一的订单起抽金额和抽成金额"), "", "ajax");
                        }
                        $serve_fee["fee_instore"]["level2"] = array("fee_start" => floatval($instore_GPC["level2"]["fee_start"]), "fee" => floatval($instore_GPC["level2"]["fee"]));
                        if (empty($serve_fee["fee_instore"]["level2"]["fee_start"]) || empty($serve_fee["fee_instore"]["level2"]["fee"])) {
                ...
                if ($serve_fee["fee_paybill"]["type"] == 2) {
                    ...
                    if ($serve_fee["fee_paybill"]["type"] == 3) {
                        $serve_fee["fee_paybill"]["level1"] = array("fee_start" => floatval($paybill_GPC["level1"]["fee_start"]), "fee" => floatval($paybill_GPC["level1"]["fee"]));
                        if (empty($serve_fee["fee_paybill"]["level1"]["fee_start"]) || empty($serve_fee["fee_paybill"]["level1"]["fee"])) {
                            imessage(error(-1, "请设置买单佣金计算方式的阶梯固定抽成阶梯一的订单起抽金额和抽成金额"), "", "ajax");
                        }
                        $serve_fee["fee_paybill"]["level2"] = array("fee_start" => floatval($paybill_GPC["level2"]["fee_start"]), "fee" => floatval($paybill_GPC["level2"]["fee"]));
                        if (empty($serve_fee["fee_paybill"]["level2"]["fee_start"]) || empty($serve_fee["fee_paybill"]["level2"]["fee"])) {
        ...
        $stores = pdo_fetchall("select id, title from " . tablename("tiny_wmall_store") . " where uniacid = :uniacid and status != 4", array(":uniacid" => $_W["uniacid"]));
                                        ...
                                        if ($form_type == "member_choice") {
                                            ...
                                            if (!empty($_GPC["choice"])) {
                                                foreach ($_GPC["choice"]["name"] as $key => $value) {
                                                    ...
                                                    $extra["member_choice"][] = array("name" => $name, "desc" => $desc, "fee" => $fee, "status" => $status);
                                                }
                                            }
                                            set_system_config('store.extra', $extra);
                                        } else {
                                            if ($form_type == "goods_setting") {
                                                ...
                                                if ($form_type == "kefu") {
                                                    $extra["kefu_status"] = intval($_GPC["kefu_status"]);
                                                    $update = array("kefu_status" => $extra["kefu_status"]);
                                                    set_system_config('store.extra', $extra);
                                                } else {
