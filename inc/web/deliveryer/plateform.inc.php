...
elseif($op == 'stat') {
	...
	if($_W['isajax'] && $_W['ispost']) {
		...
		if($_GPC['errander'] == '1') {
			$data = pdo_fetchall("SELECT addtime FROM " . tablename('tiny_wmall_errander_order') . 'WHERE uniacid = :uniacid AND deliveryer_id = :deliveryer_id and status = 3', array(':uniacid' => $_W['uniacid'], ':deliveryer_id' => $id));
			foreach($data as $da) {
			...
			$data = pdo_fetchall("SELECT addtime FROM " . tablename('tiny_wmall_order') . 'WHERE uniacid = :uniacid AND deliveryer_id = :deliveryer_id AND delivery_type = 2 and status = 5', array(':uniacid' => $_W['uniacid'], ':deliveryer_id' => $id));
			foreach($data as $da) {
				$key = date('m-d', $da['addtime']);
				if(in_array($key, array_keys($days))) {
					$datasets['flow1'][$key]++;
...
elseif($op == 'kefu') {
	...
	imessage(error(0, '修改客服状态成功'), referer(), 'ajax');
}
