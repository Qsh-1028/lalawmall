...
$store = store_fetch($sid, array("agentid", "id", "cid", "is_rest", "title", "logo", "location_x", "location_y", "invoice_status", "delivery_type", "delivery_mode", "delivery_price", "delivery_fee_mode", "delivery_areas", "delivery_time", "delivery_free_price", "pack_price", "delivery_within_days", "delivery_reserve_days", "order_note", "data", "not_in_serve_radius", "serve_radius", "storebd_id"));
if (is_error($store)) {
...
if ($ta == "index") {
    ...
    if (!empty($activityed["list"]["redPacket"])) {
        $redPacket_text = "-" . $_W["Lang"]["dollarSign"] . $activityed["list"]["redPacket"]["value"];
