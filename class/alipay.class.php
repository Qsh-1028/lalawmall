...
class AliPay
{
    ...
    public function __construct($pay_type = "wap")
    {
        ...
        $this->alipay = array("app_id" => $alipay["appid"], "rsa_type" => empty($alipay["rsa_type"]) ? "RSA" : $alipay["rsa_type"], "sign_type" => empty($alipay["sign_type"]) ? "key" : $alipay["sign_type"]);
        $this->cert = array("private_key" => $alipay["private_key"], "public_key" => $alipay["public_key"], "app_public_key" => $alipay["app_public_key"], "app_private_key" => $alipay["app_private_key"], "alipay_public_key" => $alipay["alipay_public_key"], "alipay_public_root_key" => $alipay["alipay_public_root_key"]);
    }
    ...
    {
        $keys = array("app_cert_sn" => "app_public_key", "alipay_root_cert_sn" => "alipay_public_root_key");
        $key = $keys[$key];
    ...
    public function checkCert()
    {
        ...
        if ($this->alipay["sign_type"] == "sn" && (empty($this->cert["app_public_key"]) || empty($this->cert["alipay_public_root_key"]))) {
            return error(-1, "支付宝支付证书不完整");
        }
        return true;
    }
