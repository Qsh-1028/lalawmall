define(["jquery", "bootstrap"], function($, bs) {
	...
	$(document).on("click", '.js-selectPlateformLink', function() {
		var input = $(this).data('input');
		var type = $(this).data('type') || 'plateform';
		var options = {
			type : type
		};
		irequire(["web/tiny"], function(tiny){
			tiny.selectPlateformLink(function(href) {
				if(input) {
					$(input).val(href).trigger('change');
				}
			}, options);
	...
	$(document).on("click", '.js-selectManagerLink', function() {
		var input = $(this).data('input');
		var type = $(this).data('type') || 'manager';
		var options = {
			type : type
		};
		irequire(["web/tiny"], function(tiny){
	...
	$(document).on("click", '.js-selectDeliveryerLink', function() {
		var input = $(this).data('input');
		var type = $(this).data('type') || 'deliveryer';
		var options = {
			type : type
		};
		irequire(["web/tiny"], function(tiny){
			tiny.selectDeliveryerLink(function(href) {
				if(input) {
					$(input).val(href).trigger('change');
				}
			}, options);
	...
	$(document).on("click", '.js-selectErranderPage', function() {
		...
		irequire(["web/tiny"], function(tiny){
			tiny.selectErranderPage(function(data){
				...
				if(title) {
					$(title).val(data.name).trigger('change');
				}
				if(src) {
					$(src).val(data.thumb_cn).trigger('change');
				}
				if(element) {
					$(element).find('img').attr('src', data.thumb_cn);
					$(element).find('.title').html(data.name);
				}
			}, {mutil: 0});
		});
	});
