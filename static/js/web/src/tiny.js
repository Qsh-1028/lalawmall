define(['bootstrap'], function($) {
	...
	tiny.selectPlateformLink = function(callback, options) {
		$('#select-link-modal').remove();
		$.ajax(tiny.getUrl('common/plateform'), {
			type: "get",
			dataType: "html",
			cache: false,
			data: options
	...
	tiny.selectManagerLink = function(callback, options) {
		$('#select-link-modal').remove();
		$.ajax(tiny.getUrl('common/manager'), {
			type: "get",
			dataType: "html",
			cache: false,
			data: options
	...
	tiny.selectDeliveryerLink = function(callback, options) {
		$('#select-link-modal').remove();
		$.ajax(tiny.getUrl('common/deliveryer/link'), {
			type: "get",
			dataType: "html",
			cache: false,
			data: options
		...
		$.ajax(tiny.getUrl('common/wxapp/link'), {
			...
			modal.iappend(html, function() {
				...
				$(document).off("click", "#select-wxapp-link-modal .btn-webview").on("click", "#select-wxapp-link-modal .btn-webview", function() {
					...
					if(webview.indexOf('https://') == -1 && webview.indexOf('http://') == -1) {
						webview = 'webview:https://' + webview;
					} else {
	...
	tiny.selectWxappIcon = function(callback, options) {
		...
		$.ajax(tiny.getUrl('common/wxapp/icon'), {
			...
			modal.iappend(html, function() {
				$(document).off("click", "#select-wxapp-icon-modal .tabbar-list .item").on("click", "#select-wxapp-icon-modal .tabbar-list .item", function() {
					...
					var path = '';
					if(options.type == 'deliveryer' || options.type == 'manager') {
						path = options.type + "/";
					}
					var tabbar = {
						index: index,
						url: {
							normal: '../addons/we7_wmall/plugin/wxapp/static/img/tabbar/' + path + 'icon-' + index + '.png',
							active: '../addons/we7_wmall/plugin/wxapp/static/img/tabbar/' + path + 'icon-' + index + '-active.png'
						},
						tabbar: {
							normal: 'static/img/tabbar/' + path + 'icon-' + index + '.png',
							active: 'static/img/tabbar/' + path + 'icon-' + index + '-active.png'
						}
					};
					if($.isFunction(callback)){
						callback(tabbar);
	...
	tiny.selectGohomeGoods = function(callback, option) {
		$('#select-gohomeGoods-modal').remove();
		$.ajax(tiny.getUrl('common/gohome/goods'), {
			type: "get",
			dataType: "html",
			cache: false
		}).done(function(html) {
			$Modal = $('<div class="modal fade" id="select-gohomeGoods-modal"></div>');
			$(document.body).append($Modal), $Modal.modal("show");
			$Modal.iappend(html, function(){
				if(option.mutil == 1) {
					$Modal.find('.modal-footer').removeClass('hide');
				...
				$Modal.find('#search').on('click', function(){
					var key = $.trim($Modal.find('#keyword').val());
					$.post(tiny.getUrl('common/gohome/goods'), {key: key, type: option.type, sid: option.sid}, function(data){
						var result = $.parseJSON(data);
						if(result.message.message && result.message.message.length > 0) {
							$Modal.find('.content').data('attachment', result.message.data);
								...
								laytpl(gettpl).render(result.message.message, function(html){
									...
									$Modal.find('.content .btn-item').on('click', function(){
										if(!option.mutil) {
											...
											var gohomeGoods = result.message.message[index];
											callback = eval(callback);
											if($.isFunction(callback)){
												callback(gohomeGoods, option);
											}
											$Modal.modal('hide');
										} else {
											$(this).toggleClass('btn-primary');
			...
			$Modal.find('#search').on('click', function(){
				...
				$.post(tiny.getUrl('common/account/list'), {key: key}, function(data){
					...
					if(result.message.message && result.message.message.length > 0) {
						...
						$Modal.find('.content #info').html('没有符合条件的公众号');
					}
				});
			});
		});
	...
	tiny.selectErranderPage = function(callback, option) {
		$('#select-page-modal').remove();
		$.ajax(tiny.getUrl('common/errander/page'), {
			type: "get",
			dataType: "html",
			cache: false
		}).done(function(html) {
			$Modal = $('<div class="modal fade" id="select-page-modal"></div>');
			$(document.body).append($Modal), $Modal.modal("show");
			$Modal.iappend(html, function(){
				if(option.mutil == 1) {
					$Modal.find('.modal-footer').removeClass('hide');
				...
				$Modal.find('#search').on('click', function(){
					var key = $.trim($Modal.find('#keyword').val());
					$.post(tiny.getUrl('common/errander/page'), {key: key}, function(data){
						var result = $.parseJSON(data);
						if(result.message.message && result.message.message.length > 0) {
							$Modal.find('.content').data('attachment', result.message.data);
							var gettpl = $('#select-page-data').html();
							irequire(['laytpl'], function(laytpl){
								laytpl(gettpl).render(result.message.message, function(html){
									$Modal.find('.content').html(html);
									$Modal.find('.content .btn-item').off();
							...
							$Modal.find('.content #info').html('没有符合条件的场景');
						}
					});
				});
			});
	...
	tiny.map = function(val, callback){
		...
		var mapType = val.mapType ? val.mapType : 'gaode';
		if(mapType == 'gaode') {
			...
			$.getScript('//maps.googleapis.com/maps/api/js?key=AIzaSyABxMCzgtzJxCbJu8Cxwv7BszayIAWN1xw&libraries=places', function(){
				var geo = new google.maps.Geocoder(); //地址编码类
				...
				if(modalobj.length == 0) {
					...
					var infowindow = new google.maps.InfoWindow({
						content: '<span>请您移动此标记，选择您的坐标！</span>'
					});
					...
					google.maps.event.addListener(marker, 'dragend', function(event) {
						...
						geo.geocode({
							...
							if(status == 'OK' && result && result.length > 0) {
								var addressObj = result[0];
								modalobj.find('.input-group :text').val(addressObj.formatted_address);
							}
						})
					});

					var placesService = new google.maps.places.PlacesService(map); //地址搜索类;
					function searchAddress(address) {
				...
				modalobj.find('button.btn-primary').on('click', function(){
					if($.isFunction(callback)) {
						...
						geo.geocode({
							...
							if(status == 'OK' && result && result.length > 0) {
								var addressObj = result[0];
								var latLng = addressObj.geometry.location;
								latLng = JSON.stringify(latLng);
								latLng = JSON.parse(latLng);
								var val = {lng: latLng.lng.toFixed(7), lat: latLng.lat.toFixed(7), label: addressObj.formattedAddress};
								callback(val);
							}
						})
					}
