...
class subscribe extends dianwoda 
{
	...
	public function parse() 
	{
		if( $this->type == "dianwoda.order.status-update" ) 
		{
			...
			{
				case "created": break;
				case "dispatched": $deliveryer = array( "id" => $content["rider_code"], "title" => $content["rider_name"], "mobile" => $content["rider_mobile"] );
				$status = order_deliveryer_update_status($order["id"], "delivery_assign", array( "role" => "dianwoda", "deliveryer" => $deliveryer ));
				break;
				case "arrived": $status = order_deliveryer_update_status($order["id"], "delivery_instore", array( "role" => "dianwoda" ));
				break;
				case "obtained": $status = order_deliveryer_update_status($order["id"], "delivery_takegoods", array( "role" => "dianwoda" ));
				break;
				case "completed": $status = order_status_update($order["id"], "end", array( "role" => "dianwoda" ));
				break;
				case "abnormal": break;
				case "canceled": $status = order_status_update($order["id"], "notify_deliveryer_collect", array( "force" => 1, "channel" => "re_notify_deliveryer_collect", "role" => "dianwoda" ));
				break;
			}
			if( is_error($status) ) 
			{
