...
if ($op == "post") {
    ...
    if ($_W["ispost"]) {
        ...
        $insert = array("uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "roleid" => intval($_GPC["roleid"]), "status" => intval($_GPC["status"]), "realname" => trim($_GPC["realname"]), "mobile" => trim($_GPC["mobile"]), "nickname" => trim($_GPC["nickname"]), "avatar" => trim($_GPC["avatar"]), "perms" => implode(",", $_GPC["perms"]));
        if (0 < $insert["roleid"]) {
