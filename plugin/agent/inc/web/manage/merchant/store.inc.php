...
if ($op == "list") {
    ...
    $condition = " uniacid = :uniacid and agentid = :agentid and (status = 1 or status = 0) and is_waimai = 1 ";
...
if ($op == "batch") {
    ...
    if ($type != "status") {
        ...
        pdo_query('update ' . tablename('tiny_wmall_store') . " set " . $type . " = " . $value . " where uniacid = " . $_W["uniacid"] . " and agentid = " . $_W["agentid"] . " and status not in (2, 3, 4)");
    }
    imessage(error(0, ""), "", 'ajax');
}
if ($op == "copy") {
