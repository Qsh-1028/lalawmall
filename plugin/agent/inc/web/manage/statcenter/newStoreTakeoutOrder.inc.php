...
if ($op == "index") {
    ...
    $stores = pdo_fetchall("select id, title, addtime from " . tablename("tiny_wmall_store") . " where uniacid = :uniacid and agentid = :agentid and status = :status and addtime >= :starttime and addtime < :endtime", array(":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":status" => 1, ":starttime" => $starttime, ":endtime" => $endtime), "id");
    $condition = " WHERE uniacid = :uniacid and agentid = :agentid and order_type <= 2 and addtime >= :addtime";
    $params = array(":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"], ":addtime" => min(array_column($stores, "addtime")));
    $sid = intval($_GPC["sid"]);
    if (0 < $sid) {
