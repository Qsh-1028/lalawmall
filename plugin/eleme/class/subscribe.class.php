...
class subscribe extends Eleme
{
    ...
    public function parse()
    {
        ...
        if ($type == 10) {
            ...
            $eleme = array("fee_type" => 2, "fee_rate" => $message["serviceRate"], "fee" => $message["serviceFee"], "note" => "饿了么服务费率为:" . $message["serviceRate"] . ",收取佣金:" . $_W["Lang"]["dollarSign"] . $message["serviceFee"]);
            $plateform_serve = array("fee_type" => 1, "fee_rate" => 0, "fee" => 0, "note" => "每单固定0" . $_W["Lang"]["dollarSignCn"], "eleme" => $eleme);
            if (!empty($account["fee_eleme"])) {
                if ($account["fee_eleme"]["fee_type"] == 1) {
                    ...
                    if ($platform_serve_fee < $account["fee_eleme"]["fee_min"]) {
                        ...
                        $text = "佣金小于最少抽佣金额，以最少抽佣金额 " . $_W["Lang"]["dollarSign"] . $platform_serve_fee . "计";
                    }
                    $plateform_serve = array("fee_type" => 1, "fee_rate" => $account["fee_eleme"]["fee_rate"], "fee" => $platform_serve_fee, "note" => "(饿了么平台最终入账 " . $_W["Lang"]["dollarSign"] . $message["income"] . ") x " . $account["fee_eleme"]["fee_rate"] . "% " . $text, "eleme" => $eleme);
                } else {
                    $platform_serve_fee = floatval($account["fee_eleme"]["fee"]);
                    $plateform_serve = array("fee_type" => 2, "fee_rate" => 0, "fee" => $platform_serve_fee, "note" => "每单固定" . $platform_serve_fee . $_W["Lang"]["dollarSignCn"], "eleme" => $eleme);
                }
            }
            if ($_W["is_agent"]) {
                $plateform_serve["final"] = "(代理抽取饿了么订单 " . $_W["Lang"]["dollarSign"] . $platform_serve_fee . " + 代理商配送费 " . $_W["Lang"]["dollarSign"] . $order["plateform_delivery_fee"] . " - 代理商支付给配送员配送费 " . $_W["Lang"]["dollarSign"] . $order["plateform_deliveryer_fee"] . ")";
            ...
            if (!empty($message["orderActivities"])) {
                foreach ($message["orderActivities"] as $orderActivity) {
                    $discount = array("uniacid" => $_W["uniacid"], "oid" => $order_id, "sid" => $order["sid"], "type" => "discount", "icon" => "discount_b.png", "name" => $orderActivity["name"], "note" => "-" . $_W["Lang"]["dollarSign"] . $orderActivity["amount"], "fee" => $orderActivity["amount"]);
