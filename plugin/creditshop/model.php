...
function creditshop_order_notice($orderOrId, $type, $extra = array())
{
    ...
    if ($channel == "wap") {
        ...
        if ($channel == "wxapp") {
            ...
            if ($type == "pay") {
                $params = array("keyword1" => "积分商城下单", "keyword2" => (string) $goods_types[$order["goods_type"]] . ":" . $order["goods_info"]["title"], "keyword3" => "订单" . $status_cn[$order["status"]], "keyword4" => date("Y-m-d H:i", $order["paytime"]), "keyword5" => "无");
            } else {
                if ($type == "handle") {
                    $params = array("keyword1" => "积分商城订单核销", "keyword2" => "商品:" . $order["goods_info"]["title"], "keyword3" => "订单已核销", "keyword4" => date("Y-m-d H:i", TIMESTAMP), "keyword5" => "无");
                }
            }
            $send = sys_wechat_tpl_format($params);
