	...
	if($_W['ispost']) {
		...
		foreach($data as $da) {
			...
			$insert = array(
				'uniacid' => $_W['uniacid'],
				'title' => trim($da[0]),
				//'category_id' => intval(pdo_fetchcolumn('select id from ' . tablename('tiny_wmall_cloudgoods_goods_category') . ' where uniacid = :uniacid and title = :title', array(':uniacid' => $_W['uniacid'], ':title' => $da[1]))),
				'unitname' => trim($da[2]),
				'price' => floatval($da[3]),
