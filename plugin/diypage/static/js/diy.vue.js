define(['jquery.ui'], function(ui) {
		...
		$("#parts nav").unbind('click').click(function(){
			...
			if(item.max && item.max > 0) {
				...
				if(itemNum > 0 && itemNum >= item.max) {
					Notify.error("此源素最多允许添加 " + item.max + " 个");
					return;
				}
			}
			var append = true;
		...
		diy.parts = {
					...
					C0123456789101: {
						...
						activity: {
							items: {
								...
								C0123456789102: {
									type: 'couponCollect',
									title: '可领2' + Lang.dollarSignCn + '代金券',
								}
							},
							num: 2
						},
			...
			copyright: {
				...
				params: {
					datafrom: 0,
					content: '请填写版权说明',
					imgurl: '../addons/we7_wmall/plugin/diypage/static/img/default/copyright.png',
					config: '',
				},
				style: {
					showimg: 1,
			...
			goodsTab: {
				...
				data: {
					C0123456789101: {
						TabsTitle: '品质正餐',
						imgTitle: '../addons/we7_wmall/plugin/diypage/static/template/default3/img-card-1.jpg',
						goodsdata: '0',
						goods: {
							C0123456789101: {
								sid: '0',
	...
	diy.initEditor = function(scroll) {
		...
		if(goodslength > 0) {
			...
			$(document).on("click", '.js-selectGoods', function() {
				...
				diy.parentid = $(this).closest('.item').data('parentid');
		...
		if(childitems > 0) {
			...
			$(".addStoreItem").unbind('click').click(function(){
				...
				var childName = diy.getId("M", 0);
				diy.items[itemid].data[parentId].stores[childName] = newChild;
				diy.initItems(itemid);
				diy.initEditor(false);
			});
			$(".addItem").unbind('click').click(function(){
				...
				var childName = diy.getId("M", 0);
				diy.items[itemid].data[parentId].goods[childName] = newChild;
				diy.initItems(itemid);
				diy.initEditor(false);
			});
			$("#app-editor .form-items .item .btn-del").unbind('click').click(function(){
				...
				if(min) {
					if(diy.items[itemid].id == 'goodsTab' && typeof(parentid) !== "undefined") {
						var length = diy.length(diy.items[itemid].data[parentid].goods);
					} else if(diy.items[itemid].id == 'storesTab' && typeof(parentid) !== "undefined"){
						var length = diy.length(diy.items[itemid].data[parentid].stores);
					} else {
						var length = diy.length(diy.items[itemid].data);
					}
					if(length <= min) {
				...
				Notify.confirm("确定删除吗", function(){
					if(diy.items[itemid].id == 'goodsTab' && typeof(parentid) !== "undefined") {
						delete diy.items[itemid].data[parentid].goods[childid];
					} else if(diy.items[itemid].id == 'storesTab' && typeof(parentid) !== "undefined"){
						delete diy.items[itemid].data[parentid].stores[childid];
					} else {
	...
	diy.initTpl = function(){
		...
		tmodtpl.helper("checkPluginPerm", function(name) {
			return  diy.checkPluginExist(name);
		});
		tmodtpl.helper("Lang", function(type) {
			return Lang[type];
		});
		tmodtpl.helper("LangType", function() {
			return diy.LangType;
		});
	...
	diy.callbackGoods = function(data) {
		...
		var parentid = diy.parentid;
		if(diy.items[itemid].id == 'goodsTab' && typeof(parentid) !== "undefined") {
			diy.items[itemid].data[parentid].goods[childid] = {
				sid: data.sid,
				goods_id: data.id,
				title: data.title,
				thumb: data.thumb,
	...
	diy.callbackStore = function(data) {
		...
		var childid = diy.childid;
		var parentid = diy.parentid;
		if(diy.items[itemid].id == 'storesTab' && typeof(parentid) !== "undefined") {
			diy.items[itemid].data[parentid].stores[childid] = {
				store_id: data.id,
				logo: data.logo,
				title: data.title,
