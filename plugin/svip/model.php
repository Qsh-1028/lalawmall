...
function svip_buy_notice($id, $extra = array())
{
    ...
    if ($order_channel == "wechat") {
        if (!empty($order["openid"])) {
            ...
            $remark = array("会员套餐: " . $meal["title"], "支付费用: " . $order["final_fee"] . $_W["Lang"]["dollarSignCn"], "支付方式: " . $order["pay_type_cn"], "会员期限: " . date("Y-m-d", $order["starttime"]) . "~" . date("Y-m-d", $order["endtime"]));
...
function svip_goods_getall($filter = array())
{
    ...
    $condition = " where a.uniacid = :uniacid and a.svip_status = 1 and b.status = 1";
    $params = array(":uniacid" => $_W["uniacid"]);
...
function svip_redpacket_fetchall($filter = array())
{
    ...
    if (!empty($redpackets)) {
        ...
        foreach ($redpackets as &$redpacket) {
            ...
            $limit = array();
            if ($redpacket["sid"] == 0 && !empty($redpacket["data"]["categorys"])) {
                $categorys = array();
                foreach ($redpacket["data"]["categorys"] as $rcate) {
                    if ($redpacket["data"]["scene"] == 1) {
                        if (!empty($paotui_scene[$rcate["id"]])) {
                            $categorys[] = $paotui_scene[$rcate["id"]]["name"];
                        }
                    } else {
                        if (!empty($store_categorys[$rcate["id"]])) {
                            $categorys[] = $store_categorys[$rcate["id"]]["title"];
                        }
                    }
                }
                if (!empty($categorys)) {
                    $categorys_str = implode(",", $categorys);
                    $limit[] = "限" . $categorys_str . "使用";
                }
            }
            if (!empty($redpacket["data"]["times"])) {
                $times = array();
                foreach ($redpacket["data"]["times"] as $rtime) {
                    $times[] = (string) $rtime["start_hour"] . "~" . $rtime["end_hour"];
                }
                $times_str = implode(",", $times);
                $limit[] = "限" . $times_str . "使用";
            }
            if ($redpacket["sid"] == 0 && $redpacket["data"]["scene"] == 0 && 0 < $redpacket["data"]["order_type_limit"]) {
                $order_type_limit_cn = $redpacket["data"]["order_type_limit"] == 2 ? "限到店自提单" : "限平台配送单";
                $limit[] = $order_type_limit_cn;
            }
            if (empty($limit)) {
                $redpacket["limit_cn"] = "通用红包";
            } else {
                $redpacket["limit_cn"] = implode(",", $limit);
            }
        }
    }
...
function svip_redpacket_exchage($redpacketOrId, $exchange_cost = 0)
{
    ...
    if ($exchange_cost == 1 && 0 < $redpacket["exchange_cost"] && $redpacket["can_exchange"] == 1) {
        $title = "兑换" . $redpacket["discount"] . $_W["Lang"]["dollarSignCn"] . "平台红包";
        $remark = "";
        if (0 < $redpacket["sid"]) {
            $title = "兑换" . $redpacket["discount"] . $_W["Lang"]["dollarSignCn"] . "商家红包";
    ...
    $times_limit = array();
    if (!empty($redpacket["data"]["times"])) {
        foreach ($redpacket["data"]["times"] as $time) {
    ...
    if (!empty($redpacket["data"]["categorys"])) {
        foreach ($redpacket["data"]["categorys"] as $category) {
            $category_limit[] = $category["id"];
        }
    }
    $data = array("title" => $redpacket["title"], "channel" => $channel, "type" => "grant", "scene" => $redpacket["data"]["scene"] == 1 ? "paotui" : "waimai", "order_type_limit" => intval($redpacket["data"]["order_type_limit"]), "discount" => $redpacket["discount"], "days_limit" => $redpacket["use_days_limit"], "condition" => $redpacket["condition"], "uid" => $_W["member"]["uid"], "activity_id" => $redpacket["id"], "sid" => $redpacket["sid"], "discount_bear" => $redpacket["data"]["discount_bear"], "times_limit" => iserializer($times_limit), "category_limit" => implode("|", $category_limit));
...
function svip_task_getall($filter = array())
{
    ...
    if (defined('IN_WXAPP') || defined('IN_VUE')) {
        if (!empty($tasks)) {
            $award_types = array("credit1" => "积分", "credit2" => $_W["Lang"]["dollarSignCn"], "svip_credit1" => "奖励金");
                ...
                if (!empty($val["data"]["award"])) {
                    foreach ($val["data"]["award"] as $key => $award) {
                        if ($key == "redpackets" && !empty($award)) {
                            ...
                            $val["award_cn"] .= "+" . $award[0]["discount"] . $_W["Lang"]["dollarSignCn"] . "红包 ";
                        } else {
                            if (0 < $award) {
    ...
    if (!empty($store_redpacket)) {
        ...
        if (0 < $num_taking) {
            ...
            if (!empty($plateform_redpacket)) {
                ...
                $plateform_redpacket["desc"] = "您可将此红包升级为" . $_W["Lang"]["dollarSign"] . $store_redpacket["discount"] . "红包";
        ...
        if (!empty($member_redpacket)) {
            ...
            $member_redpacket["desc"] = "您可将此红包升级为" . $_W["Lang"]["dollarSign"] . $store_redpacket["discount"] . "红包";
        }
