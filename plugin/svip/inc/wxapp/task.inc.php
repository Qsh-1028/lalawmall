...
if ($op == 'index') {
	...
	$result = array(
		'tasks'            => $data['tasks'],
		'stat'             => array('total_finish' => intval(pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall_svip_task_records') . ' where uniacid = :uniacid and uid = :uid and status = 2', array(':uniacid' => $_W['uniacid'], ':uid' => $_W['member']['uid'])))),
...
if ($op == 'takepart') {
	...
	if ($data['task_endtime_type'] == 1) {
	...
	else if ($data['task_endtime_type'] == 2) {
		...
		if ($data['task_endtime_type'] == 3) {
			...
			else if ($data['task_endtime'] == 'week') {
