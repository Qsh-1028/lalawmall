    ...
    if (0 < $overtime) {
        ...
        if (!empty($rule)) {
            ...
            if ($store["data"]["zhunshibao"]["fee_type"] == 1) {
                ...
                $log["compensate_fee"] = (string) $_W["Lang"]["dollarSign"] . $rule["fee"];
                ...
                if ($store["data"]["zhunshibao"]["fee_type"] == 2) {
                    ...
                    $log["compensate_fee"] = (string) $_W["Lang"]["dollarSign"] . $update["zhunshibao_compensate"];
...
function zhunshibao_update_status($orderOrId, $status)
{
    ...
    if ($status == 3) {
        $log = array($order["uid"], "准时宝赔偿, 订单号:" . $order["ordersn"] . ", 赔付金额:" . $order["zhunshibao_compensate"] . $_W["Lang"]["dollarSignCn"], "we7_wmall");
        mload()->model('member');
