...
if ($op == "index") {
    ...
    if ($op == "detail") {
        ...
        if (!empty($member_takeinfo)) {
            $take_status = 1;
            $_W["_share"] = array("title" => !empty($activity["share"]["share_title"]) ? $activity["share"]["share_title"] : $activity["name"], "desc" => "我正在参加最低" . $activity["price"] . $_W["Lang"]["dollarSignCn"] . "购买" . $activity["name"] . "的活动，快来帮我砍价吧", "imgUrl" => $activity["thumb"], "link" => ivurl("/gohome/pages/kanjia/share", array("activityid" => $activity["id"], "uid" => $member_takeinfo["uid"]), true));
            $sharedata = array("title" => !empty($activity["share"]["share_title"]) ? $activity["share"]["share_title"] : $activity["name"], "desc" => "我正在参加最低" . $activity["price"] . $_W["Lang"]["dollarSignCn"] . "购买" . $activity["name"] . "的活动，快来帮我砍价吧", "imageUrl" => $activity["thumb"], "path" => "/gohome/pages/kanjia/share?activityid=" . $activity["id"] . "&uid=" . $member_takeinfo["uid"]);
        } else {
        ...
        if ($op == "create") {
            ...
            if ($op == "share") {
                ...
                $_W["_share"] = array("title" => !empty($activity["share"]["share_title"]) ? $activity["share"]["share_title"] : $activity["name"], "desc" => "我正在参加最低" . $activity["price"] . $_W["Lang"]["dollarSignCn"] . "购买" . $activity["name"] . "的活动，快来帮我砍价吧", "imgUrl" => $activity["thumb"], "link" => ivurl("/gohome/pages/kanjia/share", array("activityid" => $activity["id"], "uid" => $takeinfo["uid"]), true));
                $sharedata = array("title" => !empty($activity["share"]["share_title"]) ? $activity["share"]["share_title"] : $activity["name"], "desc" => "我正在参加最低" . $activity["price"] . $_W["Lang"]["dollarSignCn"] . "购买" . $activity["name"] . "的活动，快来帮我砍价吧", "imageUrl" => $activity["thumb"], "path" => "/gohome/pages/kanjia/share?activityid=" . $activity["id"] . "&uid=" . $takeinfo["uid"]);
