...
function gohome_order_update_bill($orderOrId)
{
    ...
    $agent_serve["final"] = "(代理商抽取佣金 " . $_W["Lang"]["dollarSign"] . $platform_serve_fee . " - 平台服务佣金 " . $_W["Lang"]["dollarSign"] . $agent_serve_fee . " - 代理商补贴 " . $_W["Lang"]["dollarSign"] . $order["agent_discount_fee"] . ")";
...
function gohome_order_begin_refund($orderOrid)
{
    ...
    if ($order["pay_type"] == "credit") {
        if (0 < $order["uid"]) {
            $log = array($order["uid"], (string) $order["order_type_cn"] . "订单退款, 订单号:" . $order["id"] . ", 退款金额:" . $order["final_fee"] . $_W["Lang"]["dollarSignCn"], "we7_wmall");
            mload()->model('member');
...
function gohome_order_status_notice($orderOrId, $type, $extra = array())
{
    ...
    if ($order_channel == "wechat") {
        ...
        $params_send = array("title" => $title, "ordersn" => $order["ordersn"], "final_fee" => $order["final_fee"], "pay_type_cn" => $order["pay_type_cn"], "delivery_title" => "******", "status_cn" => $order["status_cn"], "remark" => $remark);
...
function gohome_complain_notice($idOrComplain)
{
    ...
    if (!empty($complain)) {
        ...
        $remark = implode("\n", $remark);
        $url = $complain["link"];
        $params_send = array("title" => $title, "ordersn" => "******", "final_fee" => "******", "pay_type_cn" => "******", "delivery_title" => "******", "status_cn" => "******", "remark" => $remark);
        $send = tpl_format($params_send);
...
function gohome_order_clerk_notice($id, $type, $note = "")
{
    ...
    if ($type == "pay") {
        $title = "您的店铺有新的" . $order["order_type_cn"] . "订单,订单号为 #" . $order["id"] . " ,订单金额:" . $order["final_fee"] . $_W["Lang"]["dollarSignCn"] . ",请尽快处理";
        ...
        if (!empty($li["print_no"])) {
            ...
            if (!empty($order["goods"])) {
                ...
                if ($order["order_type"] == "kanjia") {
                    ...
                    $content[] = "好友累计帮砍:" . $bargain . $_W["Lang"]["dollarSignCn"];
                } else {
                    if ($order["order_type"] == "pintuan") {
