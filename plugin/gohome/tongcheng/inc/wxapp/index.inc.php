...
if($op == 'index') {
	...
	if($_config_wxapp['diy']['use_diy_tongcheng'] != 1) {
		...
		$share = array(
			...
			'link' => empty($config_share['link']) ? ivurl('gohome/pages/tongcheng/index', array(), true) : $config_share['link'],
	...
	$page = get_wxapp_diy($pageOrid, true, array('pagepath' => 'tongcheng'));
	if(empty($page)) {
	...
	$_W['_share'] = array(
		...
		'link' => ivurl('gohome/pages/tongcheng/index', array(), true),
...
elseif($op == 'detail') {
	...
	$_W['_share'] = array(
		'title' => cutstr($information['content_share'], 20, true),
		'desc' => cutstr($information['content_share'], 50, true),
		'link' => ivurl('gohome/pages/tongcheng/detail', array('id' => $id), true),
		'imgUrl' => $information['thumbs'][0]
	);
	$result = array(
		'detail' => $information,
		'comments' => $comments,
...
elseif($op == 'category') {
	...
	$_W['_share'] = array(
		'title' => $categorys[$cid]['title'],
		'desc' => $categorys[$cid]['content'],
		'link' => ivurl('gohome/pages/tongcheng/category', array('id' => $cid, 'childid' => $childid), true),
		'imgUrl' => $categorys[$cid]['thumb']
	);
	imessage(error(0, $result), '', 'ajax');
}
...
elseif($op == 'reply') {
	...
	$update = array(
		...
		'from_nickname' => $_W['member']['nickname'],
		'from_avatar' => $_W['member']['avatar'],
		'to_uid' => $to_member['uid'],
		'to_nickname' => $to_member['nickname'],
		'to_avatar' => $to_member['avatar'],
		'addtime' => TIMESTAMP,
	);
	pdo_insert('tiny_wmall_tongcheng_reply', $update);
	$extra = array(
		'content' => '回复:' . $update['content'],
		'nickname' => $update['from_nickname'],
		'addtime' => $update['addtime'],
...
elseif($op == 'like') {
	...
	$update = array(
		'like_uid' => iserializer($like_uid),
		'likenum' => $information['likenum'] + 1,
	);
...
elseif($op == 'get_search') {
	$result = array(
		'categorys' => pdo_getall('tiny_wmall_tongcheng_category', array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'status' => 1, 'is_hot' => 1), array('id', 'parentid', 'title'))
	);
	imessage(error(0, $result), '', 'ajax');
}
