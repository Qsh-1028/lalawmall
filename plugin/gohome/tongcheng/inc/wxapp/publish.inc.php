    ...
    else
    {
        if( $op == "stick" ) 
        {
            ...
            if( $_W["ispost"] ) 
            {
                if( 0 < $calculate["final_fee"] ) 
                {
                    $update_trade = array( "uniacid" => intval($_W["uniacid"]), "agentid" => $_W["agentid"], "tid" => $information_id, "uid" => $_W["member"]["uid"], "type" => 2, "price" => 0, "stick_price" => $calculate["stick_price"], "final_fee" => $calculate["final_fee"], "days" => $calculate["days"], "ordersn" => date("YmdHis") . random(6, true), "addtime" => TIMESTAMP, "stat_day" => date("Ymd", TIMESTAMP) );
                    $update_trade = tongcheng_tiezi_order_bill($update_trade);
                    pdo_insert("tiny_wmall_tongcheng_order", $update_trade);
                    $trade_id = pdo_insertid();
