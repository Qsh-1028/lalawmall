...
if ($op == "list") {
    ...
    if ($op == "post") {
        ...
        $agentid = !empty($store) ? $store["agentid"] : $_W["agentid"];
        $categorys = pdo_fetchall("select * from " . tablename("tiny_wmall_haodian_category") . " where uniacid = :uniacid and agentid = :agentid order by displayorder desc", array(":uniacid" => $_W["uniacid"], ":agentid" => $agentid), "id");
        if (!empty($categorys)) {
            foreach ($categorys as &$val) {
                $val["name"] = $val["title"];
                if (empty($val["parentid"])) {
