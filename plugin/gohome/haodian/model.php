    ...
    foreach ($table as $key => $value) {
        ...
        if (!empty($temp)) {
            if ($key == "kanjia") {
                ...
                $data[$key]["text"] = (string) $temp["name"] . " 原价" . $_W["Lang"]["dollarSign"] . $temp["oldprice"] . " 邀请好友帮忙砍价最低" . $_W["Lang"]["dollarSign"] . $temp["price"] . "即可购买";
            } else {
                if ($key == "pintuan") {
                    ...
                    if ($key == "seckill") {
                        ...
                        $data[$key]["text"] = (string) $temp["name"] . " 原价" . $_W["Lang"]["dollarSign"] . $temp["oldprice"] . " 限时抢购价" . $_W["Lang"]["dollarSign"] . $temp["price"] . "即可购买";
                    }
                }
            }
        }
...
function haodian_settle_order_bill($order)
{
    ...
    if (0 < $order["agentid"]) {
        ...
        if ($agent_fee_config["type"] == 2) {
            ...
            $agent_serve = array("fee_type" => 2, "fee_rate" => 0, "fee" => $agent_serve_fee, "note" => "固定抽成" . $agent_serve_fee . $_W["Lang"]["dollarSignCn"]);
        } else {
