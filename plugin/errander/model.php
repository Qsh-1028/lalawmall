    ...
    if (!empty($order["openid"])) {
        ...
        if ($order_channel == "wap") {
            if ($type == "pay") {
                ...
                $remark = array("订单类型: " . $order["order_type_cn"], "商品信息: " . $order["goods_name"], "总金　额: " . $order["total_fee"] . $_W["Lang"]["dollarSignCn"], "支付方式: " . $order["pay_type_cn"], "支付时间: " . date("Y-m-d H: i", $order["paytime"]));
            } else {
            ...
            $remark = implode("\n", $remark);
            $params_send = array("title" => $title, "ordersn" => $order["order_sn"], "final_fee" => $order["final_fee"], "pay_type_cn" => $order["pay_type_cn"], "delivery_title" => $_W["we7_wmall"]["config"]["mall"]["delivery_title"], "status_cn" => $order["status_cn"], "remark" => $remark);
...
function errander_order_deliveryer_notice($id, $type, $deliveryer_id = 0, $note = "")
{
    ...
    if ($type == "new_delivery") {
        ...
        $remark = array("下单时间: " . date("Y-m-d H:i", $order["addtime"]), "配送　费: " . $order["deliveryer_fee"] . $_W["Lang"]["dollarSignCn"], "小　　费: " . $order["delivery_tips"] . $_W["Lang"]["dollarSignCn"], "本单收入: " . ($order["deliveryer_fee"] + $order["delivery_tips"]) . $_W["Lang"]["dollarSignCn"], "收货　人: " . $order["accept_username"], "送货地址: " . $order["accept_address"]);
        $remark = implode("\n", $remark);
        ...
        if ($type == "delivery_wait") {
            ...
            if ($type == "cancel") {
                ...
                if ($type == "direct_transfer") {
                    ...
                    $remark = array("下单时间: " . date("Y-m-d H:i", $order["addtime"]), "转单时间: " . date("Y-m-d H:i", TIMESTAMP), "小　　费: " . $order["delivery_tips"] . $_W["Lang"]["dollarSignCn"], "收货　人: " . $order["accept_username"], "送货地址: " . $order["accept_address"], "订单类型: " . $order["order_type_cn"]);
                    $remark = implode("\n", $remark);
    ...
    if ($channel_notice == "wechat") {
        ...
        $data = array($title, "******", "#" . $order["serial_cn"], "跑腿单", date("Y-m-d H:i", $order["addtime"]), $order["note"], (string) $_W["Lang"]["dollarSign"] . $order["final_fee"]);
        ...
        if ($type == "pay") {
            ...
            if ($type == "cancel") {
                ...
                if (!$order["is_pay"] || $order["final_fee"] <= 0) {
                    ...
                    $note = array("取消原因: " . $extra["note"], "退款金额: " . $order["final_fee"] . $_W["Lang"]["dollarSignCn"], "已付款项会在1-3工作日内返回您的账号");
...
function errander_order_begin_payrefund($id)
{
    ...
    if ($order["pay_type"] == "credit") {
        if (0 < $order["uid"]) {
            $log = array($order["uid"], "外送模块订单退款, 订单号:" . $order["id"] . ", 退款金额:" . $order["final_fee"] . $_W["Lang"]["dollarSignCn"], "we7_wmall");
            mload()->model('member');
...
function errander_order_calculate_delivery_fee($idOrCategory, $extra, $is_calculate = 0)
{
    ...
    $fees = array();
    $fees["basic"] = array("title" => "基础配送费", "note" => "固定金额", "fee" => $distance_fee, "fee_cn" => (string) $_W["Lang"]["dollarSign"] . $distance_fee);
    if ($rule_fee_type != "fee") {
        ...
        if (!empty($rule_fee_wrap["data"])) {
            foreach ($rule_fee_wrap["data"] as $row) {
                ...
                if (strtotime($row["start_hour"]) <= TIMESTAMP && TIMESTAMP < strtotime($row["end_hour"]) + 60) {
                    $rule_fee_item = $row;
                    break;
                }
            }
        ...
        if ($rule_fee_type == "distance") {
            ...
            $fees["basic"] = array("title" => "基础配送费", "note" => "里程计价", "fee" => $rule_fee_item["start_fee"], "fee_cn" => (string) $_W["Lang"]["dollarSign"] . $rule_fee_item["start_fee"]);
            if (0 < $distance_over_fee) {
                $fees[] = array("title" => "距离附加费", "note" => "", "fee" => $distance_over_fee, "fee_cn" => (string) $_W["Lang"]["dollarSign"] . $distance_over_fee);
            }
        } else {
            if ($rule_fee_type == "section") {
                ...
                $fees["basic"] = array("title" => "基础配送费", "note" => "区间计价", "fee" => $distance_fee, "fee_cn" => (string) $_W["Lang"]["dollarSign"] . $distance_fee);
            }
        }
    }
    $weight_fee = 0;
    ...
    if ($rule_fee["weight_status"] == 1) {
        ...
        if (0 < $weight_fee) {
            $fees[] = array("title" => "重量附加费", "note" => "", "fee" => $weight_fee, "fee_cn" => (string) $_W["Lang"]["dollarSign"] . $weight_fee);
        }
    }
    $deliveryinfo = array();
    ...
    if ($rule_fee["extra_fee_time_status"] == 1) {
        ...
        if (0 < $special_time_fee) {
            $fees[] = array("title" => "特殊时段附加费", "note" => "", "fee" => $special_time_fee, "fee_cn" => (string) $_W["Lang"]["dollarSign"] . $special_time_fee);
        }
    }
    $delivery_tips = 0;
    if ($basic["showtips"] == 1) {
        ...
        if (0 < $delivery_tips) {
            $fees[] = array("title" => "小费", "note" => "", "fee" => $delivery_tips, "fee_cn" => (string) $_W["Lang"]["dollarSign"] . $delivery_tips);
        }
    }
    ...
    if (!empty($activityed["redPacket"])) {
        ...
        $fees[] = array("title" => "红包", "note" => "", "fee" => 0 - $activityed["redPacket"]["discount"], "fee_cn" => "-" . $_W["Lang"]["dollarSign"] . $activityed["redPacket"]["discount"]);
        ...
        if (!empty($extra["extra_fee"])) {
            ...
            foreach ($extra["extra_fee"] as $key => $val) {
                ...
                foreach ($val as $v) {
                    ...
                    $fees[] = array("title" => (string) $rule_fee["extra_fee"][$key]["title"] . "-" . $extra_fee_name, "note" => $extra_fee_name, "fee" => $extra_item_fee, "fee_cn" => (string) $_W["Lang"]["dollarSign"] . $extra_item_fee);
                }
            ...
            foreach ($diypage["data"]["fees"]["extra_fee"] as $pindex => $item) {
                if ($item["status"] == 1) {
                    ...
                    foreach ($extra_fee_store[$pindex] as $val) {
                        ...
                        $extra_fee += $extra_item_fee;
                        $fees[] = array("title" => (string) $rule_fee["extra_fee"][$pindex]["title"] . "-" . $extra_fee_name, "note" => $extra_fee_name, "fee" => $extra_item_fee, "fee_cn" => (string) $_W["Lang"]["dollarSign"] . $extra_item_fee);
                    }
                    $extra_fees += $extra_fee;
                } else {
...
function errander_order_calculate_deliveryer_fee($order, $deliveryerOrid = 0)
{
    ...
    if (!empty($special_deliveryer_errander) && $special_deliveryer_errander["status"] == 1) {
        if ($special_deliveryer_errander["type"] == 1 && 0 < $special_deliveryer_errander["fee"]) {
            $plateform_errander_fee += floatval($special_deliveryer_errander["fee"]);
        } else {
            if ($special_deliveryer_errander["type"] == 2 && 0 < $special_deliveryer_errander["rate"]) {
...
function errander_order_count_activity($delivery_fee = 0, $errander_category = array(), $extra = array())
{
    ...
    if (!empty($extra["redpacket_id"])) {
        ...
        if (!is_error($redpacket)) {
            $activityed["list"]["redPacket"] = array("text" => "-" . $_W["Lang"]["dollarSign"] . $redpacket["discount"], "value" => $redpacket["discount"], "type" => "redPacket", "name" => "平台红包优惠", "icon" => "redPacket_b.png", "redPacket_id" => $redpacket["id"], "plateform_discount_fee" => $redpacket["discount"], "agent_discount_fee" => 0, "store_discount_fee" => 0);
...
function errander_order_update_bill($id, $extra = array())
{
    ...
    $plateform_serve = array("fee" => $plateform_serve_fee, "note" => "订单配送费 " . $_W["Lang"]["dollarSign"] . $order["delivery_fee"] . " + 订单小费 " . $_W["Lang"]["dollarSign"] . $order["delivery_tips"] . " - 使用红包 " . $_W["Lang"]["dollarSign"] . $order["discount_fee"]);
    if (0 < $order["agentid"]) {
        mload()->model("agent");
        ...
        if ($agent_fee_config["type"] == 2) {
            ...
            if ($agent_fee_config["type"] == 3) {
                ...
                $text = "(本单代理佣金" . $_W["Lang"]["dollarSign"] . $plateform_serve_fee . " - 代理商支付给配送员配送费含订单小费 " . $_W["Lang"]["dollarSign"] . $deliveryer_total_fee . ") x " . $agent_serve_rate . "%";
        ...
        $agent_serve["final"] = "(代理商抽取佣金 " . $_W["Lang"]["dollarSign"] . $plateform_serve_fee . " - 平台服务佣金 " . $_W["Lang"]["dollarSign"] . $agent_serve_fee . " - 代理商补贴 " . $_W["Lang"]["dollarSign"] . $order["agent_discount_fee"] . " - 代理商支付给配送员配送费含订单小费 " . $_W["Lang"]["dollarSign"] . $deliveryer_total_fee . ")";
        ...
        $plateform_serve["note"] .= " - 支付给配送员配送费含订单小费 " . $_W["Lang"]["dollarSign"] . $deliveryer_total_fee;
    }
