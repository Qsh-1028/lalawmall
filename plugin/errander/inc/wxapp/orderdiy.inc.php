...
if ($op == "create") {
    ...
    if (empty($category)) {
        imessage(error(-1000, "跑腿类型不存在"), "", "ajax");
    }
    ...
    $data["plateform_serve"] = iserializer(array("fee" => $data["plateform_serve_fee"], "note" => "订单配送费 " . $_W["Lang"]["dollarSign"] . $data["delivery_fee"] . " + 订单小费 " . $_W["Lang"]["dollarSign"] . $data["delivery_tips"] . " - 使用红包 " . $_W["Lang"]["dollarSign"] . $data["discount_fee"]));
