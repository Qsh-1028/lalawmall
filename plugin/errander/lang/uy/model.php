...
function errander_delivery_times($idOrCategory)
{
    ...
    if (!empty($times)) {
        foreach ($times as $key => &$row) {
            ...
            $row["delivery_price_cn"] = "يەتكۈزۈش ھەققى" . $row["delivery_price"] . $_W["Lang"]["dollarSignCn"] . "ئېستىمال";
            $end = explode(":", $row["end"]);
        ...
        $i = $start;
        while ($i < $end) {
            $category["delivery_price_cn"] = "يەتكۈزۈش ھەققى" . $category["start_fee"] . $_W["Lang"]["dollarSignCn"] . "ئېستىمال";
...
function errander_order_delivery_fee($idOrCategory, $extra)
{
    ...
    if ($category["type"] == "multiaddress") {
        ...
        for ($i = 0; $i < $start_address_num; $i++) {
            ...
            $num = $i + 1;
            $message[] = "第" . $num . "个购买地址收取" . $multiaddress["fee"][$i] . $_W["Lang"]["dollarSignCn"] . "يەتكۈزۈش ھەققى";
        }
    ...
    if (0 < $tip) {
        $fees[] = array("title" => "قوشۇمچە ھەق", "note" => "", "fee" => (string) $tip, "fee_cn" => (string) $_W["Lang"]["dollarSign"] . $tip);
    }
    $activityed = array();
    $discount_fee = 0;
...
{
    $data = array(array("css" => "", "text" => "ھەممىسى", "color" => ""), array("css" => "label label-default", "text" => "قوبۇللىنۋاتىدۇ", "color" => ""), array("css" => "label label-info", "text" => "بىر تەرەپ قىلىنۋاتىدۇ", "color" => "color-info"), array("css" => "label label-success", "text" => "تاماملاندى", "color" => "color-success"), array("css" => "label label-danger", "text" => "قالدۇرۇلدى", "color" => "color-danger"));
    return $data;
}
function errander_order_delivery_status()
{
    $data = array("1" => array("css" => "label label-default", "text" => "قوبۇللىنۋاتىدۇ", "color" => ""), "2" => array("css" => "label label-info", "text" => "تاۋارنى ساقلاۋاتىدۇ", "color" => "color-info"), "3" => array("css" => "label label-warning", "text" => "يەتكۈزۋاتىدۇ", "color" => "color-warning"), "4" => array("css" => "label label-success", "text" => "تاماملاندى", "color" => "color-success"));
    return $data;
}
function errander_order_insert_status_log($id, $type, $note = "", $extra = array())
{
    ...
    $notes = array("place_order" => array("status" => 1, "title" => "زاكاسقا پۇل تۆلەندى", "note" => "زاكاس نۇمۇرى:" . $order["order_sn"], "ext" => array(array("key" => "pay_time_limit", "title" => "پۇل تۆلەۋاتىدۇ", "note" => "زاكاسنى يوللاپ" . $config["pay_time_limit"] . "مىنۇت ئىچىدە پۇل تۆلەڭ"))), "pay" => array("status" => 2, "title" => "زاكاسقا پۇل تۆلەندى", "note" => "زاكاسقا پۇل تۆلەنگەن ۋاقىتى:" . date("Y-m-d H:i:s", $order["paytime"]), "ext" => array(array("key" => "handle_time_limit", "title" => "قوبۇللىنۋاتىدۇ", "note" => "زاكاسنى" . $config["handle_time_limit"] . "مىنۇت ئىچىدە قوبۇللىمىسا，ئاپتۇماتىك قالدۇرلىدۇ"))), "delivery_assign" => array("status" => 3, "title" => "قوبۇللاندى", "note" => ""), "delivery_instore" => array("status" => 4, "title" => "تاۋارنى ئالدى", "note" => ""), "end" => array("status" => 5, "title" => "زاكاس تاماملاندى", "note" => "ھەر قانداق پىكىرىڭىز بولسا ئالاقىلشىڭ"), "cancel" => array("status" => 6, "title" => "زاكاس قالدۇرۇلدى", "note" => ""), "delivery_transfer" => array("status" => 7, "title" => "يەتكۈزگۈچى زاكاسنى يۆتكىدى", "note" => ""), "direct_transfer" => array("status" => 8, "title" => "يەتكۈزگۈچى زاكاسنى باشقىلارغا يۆتكىدى", "note" => ""), "direct_transfer_agree" => array("status" => 9, "title" => "يەتكىۈزگۈچى زاكاسنى قوبۇللىدى", "note" => ""), "direct_transfer_refuse" => array("status" => 10, "title" => "يەتكۈزگۈچى زاكاسنى رەت قىلدى", "note" => ""));
