...
class subscribe extends Meituan
{
    ...
    public function parse()
    {
        ...
        if ($type == "order") {
            ...
            $plateform_serve = array("fee_type" => 1, "fee_rate" => 0, "fee" => 0, "note" => "每单固定0" . $_W["Lang"]["dollarSignCn"], "meituan" => $meituan);
            if (!empty($account["fee_meituan"])) {
                if ($account["fee_meituan"]["fee_type"] == 1) {
                    ...
                    $plateform_serve = array("fee_type" => 1, "fee_rate" => $account["fee_meituan"]["fee_rate"], "fee" => $platform_serve_fee, "note" => "(美团平台最终入账 " . $_W["Lang"]["dollarSign"] . $order["meituan_store_final_fee"] . ") x " . $account["fee_meituan"]["fee_rate"] . "% " . $text, "meituan" => $meituan);
                } else {
                    $platform_serve_fee = floatval($account["fee_meituan"]["fee"]);
                    $plateform_serve = array("fee_type" => 2, "fee_rate" => 0, "fee" => $platform_serve_fee, "note" => "每单固定" . $platform_serve_fee . $_W["Lang"]["dollarSignCn"], "meituan" => $meituan);
                }
            }
            if ($_W["is_agent"]) {
                $plateform_serve["final"] = "(代理抽取美团订单 " . $_W["Lang"]["dollarSign"] . $platform_serve_fee . " + 代理商配送费 " . $_W["Lang"]["dollarSign"] . $order["plateform_delivery_fee"] . " - 代理商支付给配送员配送费 " . $_W["Lang"]["dollarSign"] . $order["plateform_deliveryer_fee"] . ")";
            ...
            if (!empty($message["extras"])) {
                foreach ($message["extras"] as $orderActivity) {
                    $discount = array("uniacid" => $_W["uniacid"], "oid" => $order_id, "sid" => $order["sid"], "type" => "discount", "icon" => "discount_b.png", "name" => $orderActivity["remark"], "note" => "-" . $_W["Lang"]["dollarSign"] . $orderActivity["reduce_fee"], "fee" => $orderActivity["reduce_fee"], "store_discount_fee" => $orderActivity["poi_charge"], "plateform_discount_fee" => $orderActivity["mt_charge"]);
