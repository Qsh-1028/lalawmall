...
if ($op == "transfers") {
    ...
    if ($log["status"] == 1) {
        imessage(error(-1, "本次提现已成功, 无法打款"), referer(), "ajax");
    } else {
        if ($log["status"] == 3) {
    ...
    if (in_array($channel, array("weixin", "wxapp"))) {
        ...
        if ($channel == "bank") {
            mload()->classs("wxpay");
            $pay = new WxPay();
            $params = array("partner_trade_no" => $log["trade_no"], "enc_bank_no" => $log["account"]["account"], "enc_true_name" => $log["account"]["realname"], "bank_code" => $log["account"]["id"], "amount" => $log["final_fee"] * 100, "desc" => (string) $log["account"]["realname"] . date("Y-m-d H:i", $log["addtime"]) . "推广佣金提现申请");
            $response = $pay->mktPayBank($params);
        } else {
            if ($channel == "alipay") {
                mload()->classs("alipay");
                $pay = new AliPay();
                $params = array("out_biz_no" => $log["trade_no"], "payee_account" => $log["account"]["account"], "amount" => $log["final_fee"], "payee_real_name" => $log["account"]["realname"], "remark" => (string) $log["account"]["realname"] . date("Y-m-d H:i", $log["addtime"]) . "推广佣金提现申请");
