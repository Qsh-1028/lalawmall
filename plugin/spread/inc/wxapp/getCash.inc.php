...
if ($op == "application") {
    ...
    if ($status == 1) {
        ...
        if ($channel == "wechat") {
            ...
            $getcash_account = array("realname" => $member["realname"], "openid" => $openid, "avatar" => $member["avatar"], "nickname" => $member["nickname"], "openid_wxapp" => $member["openid_wxapp"]);
        } else {
            if ($channel == "bank") {
        ...
        $data = array("uniacid" => $_W["uniacid"], "spreadid" => $_W["member"]["uid"], "trade_no" => date("YmdHis") . random(10, true), "get_fee" => $get_fee, "take_fee" => $take_fee, "final_fee" => $final_fee, "channel" => $channel, "account" => iserializer($getcash_account), "status" => 2, "addtime" => TIMESTAMP);
