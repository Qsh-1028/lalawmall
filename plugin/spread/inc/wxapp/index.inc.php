...
$result = array("member" => $member, "commission" => intval(pdo_fetchcolumn("select count(*) from" . tablename("tiny_wmall_spread_getcash_log") . " where uniacid = :uniacid and spreadid = :spreadid", array(":uniacid" => $_W["uniacid"], ":spreadid" => $_W["member"]["uid"]))), "spread" => spread_commission_stat(), "order" => intval(pdo_fetchcolumn("select count(*) from" . tablename("tiny_wmall_order") . $condition, $params)), "down" => $down, "current" => intval(pdo_fetchcolumn("select count(*) from" . tablename("tiny_wmall_spread_current_log") . "where uniacid = :uniacid and spreadid = :spreadid", array(":uniacid" => $_W["uniacid"], ":spreadid" => $_W["member"]["uid"]))), "upgrade_explain" => get_config_text("spread:upgrade_explain"), "basic" => $basic, "rank" => $rank);
