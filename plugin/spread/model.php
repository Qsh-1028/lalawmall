...
function get_spread($spread_id = 0)
{
    ...
    if (!empty($spread)) {
        ...
        $spread["group"] = $group;
        $spread["avatar"] = tomedia($spread["avatar"]);
    }
    return $spread;
}
    ...
    if ($balance == 1) {
        if (!empty($commission)) {
            ...
            if (0 <= $fee_spread1) {
                ...
                if ($commission["commission1_type"] == "ratio") {
                    $extra["remark"] = (string) $router["order_type_cn"] . " 订单号:" . $order["id"] . ", 一级下线佣金费率:" . $commission["spread1_rate"] . ", 佣金:" . $fee_spread1 . $_W["Lang"]["dollarSignCn"];
                } else {
                    if ($commission["commission1_type"] == "fixed") {
                        $extra["remark"] = (string) $router["order_type_cn"] . " 订单号:" . $order["id"] . ", 一级下线佣金:" . $fee_spread1 . $_W["Lang"]["dollarSignCn"];
                    }
                }
            ...
            if (0 <= $fee_spread2) {
                $extra = array("trade_type" => 1, "extra" => $order["id"], "order_type" => $type);
                if ($commission["commission2_type"] == "ratio") {
                    $extra["remark"] = (string) $router["order_type_cn"] . " 订单号:" . $order["id"] . ", 二级下线佣金费率:" . $commission["spread2_rate"] . ", 佣金:" . $fee_spread2 . $_W["Lang"]["dollarSignCn"];
                } else {
                    if ($commission["commission2_type"] == "fixed") {
                        $extra["remark"] = (string) $router["order_type_cn"] . " 订单号:" . $order["id"] . ", 二级下线佣金:" . $fee_spread2 . $_W["Lang"]["dollarSignCn"];
                    }
                }
