...
function card_setmeal_buy_notice($order_id, $note = "")
{
    ...
    if (!empty($maneger["openid"])) {
        ...
        $remark = array("购卡费用: " . $order["final_fee"] . $_W["Lang"]["dollarSignCn"], "支付方式: " . $order["pay_type_cn"], "购买　人: " . ($member["realname"] ? $member["realname"] : $member["nickname"]), "联系方式: " . $member["mobile"], "购买套餐: " . $card["title"], "套餐期限: " . date("Y-m-d", $order["starttime"]) . "~" . date("Y-m-d", $order["endtime"]), "购买时间: " . date("Y-m-d H:i:s", $order["paytime"]));
    ...
    if (!empty($order["openid"])) {
        ...
        $remark = array("购卡费用: " . $order["final_fee"] . $_W["Lang"]["dollarSignCn"], "支付方式: " . $order["pay_type_cn"], "购买套餐: " . $card["title"], "套餐期限: " . date("Y-m-d", $order["starttime"]) . "~" . date("Y-m-d", $order["endtime"]), "购买时间: " . date("Y-m-d H:i:s", $order["paytime"]));
        $params = array("first" => $tips, "OrderSn" => $order["ordersn"], "OrderStatus" => "已生效", "remark" => implode("\n", $remark));
