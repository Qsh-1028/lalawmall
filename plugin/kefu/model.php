...
function kefu_get_available_chat($kefu, $fans, $relation = 'member2kefu', $extra = array())
{
    ...
    $status_arr = array('kefu' => array('key' => 'kefu_status', 'title' => '平台'), 'clerk' => array('key' => 'store_status', 'title' => '门店'), 'deliveryer' => array('key' => 'deliveryer_status', 'title' => '配送员'));
                ...
                if (!empty($deliveryer)) {
                    if (empty($deliveryer['work_status'])) {
                        ...
                        if (!in_array($deliveryer['kefu_status'], array(1, 2))) {
                            ...
                            $params = array(':uniacid' => $_W['uniacid'], ':uid' => $fans['uid'], ':deliveryer_id' => $kefu['deliveryer_id'], ':status' => 4, ':starttime' => TIMESTAMP - 3 * 24 * 3600, ':endtime' => TIMESTAMP);
    ...
    if (!empty($chat)) {
        ...
        if ($status > 0) {
            $update = array('status' => $status, 'reason' => $reason);
            if ($orderid > 0) {
    ...
    if ($status > 0) {
        $chat = array('uniacid' => $_W['uniacid'], 'relation' => $relation, 'fansopenid' => $fans['token'], 'fansavatar' => $fans['avatar'], 'fansnickname' => $fans['nickname'], 'kefuunionid' => $kefu['unionid'], 'kefuopenid' => $kefu['token'], 'kefuavatar' => $kefu['avatar'], 'kefunickname' => $kefu['nickname'], 'orderid' => $extra['orderid'], 'status' => $status, 'reason' => $reason);
        if ($_W['kefu']['user']['role'] == 'member') {
            $chat['data'] = iserializer(array('fanssystem' => get_agent_os()));
        }
...
function kefu_get_chat_log($chatid, $fans)
{
    ...
    $params = array(':uniacid' => $_W['uniacid'], ':chatid' => $chatid);
    if ($id > 0) {
        $condition .= ' and id < :id';
        $params[':id'] = $id;
    }
    ...
    $result = array('logs' => array_values($logs), 'min' => $min, 'total' => $total, 'pagetotal' => $pagetotal);
    return $result;
}
...
function kefu_add_chatlog($chat)
{
    ...
    if ($chat['touser']['kefu_id'] > 0) {
        ...
        if (is_error($status)) {
            $flag = 0;
            //不在营业时间进行非工作时间回复, 不再检测是否是忙时回复
            $log = array('type' => 'text', 'content' => $status['message'], 'send2from' => 1);
    ...
    if ($flag == 1 && !empty($insert) && $chat['fromfans'] == 1) {
        if ($chat['touser']['kefu_status'] == 2) {
            if (!empty($chat['touser']['busy_reply'])) {
                $log = array('type' => 'text', 'content' => $chat['touser']['busy_reply'], 'send2from' => 1);
                $chat_reserve = kefu_get_chat($chat, true);
                kefu_insert_chatlog($chat_reserve, $log);
            }
        }
...
function kefu_insert_chatlog($chat, $log = array())
{
    ...
    if (empty($log)) {
        $log = array('content' => trim($_GPC['content']), 'type' => trim($_GPC['type']));
    }
    $insert = array('uniacid' => $_W['uniacid'], 'chatid' => $chat['id'], 'openid' => $chat['fromopenid'], 'nickname' => $chat['fromuser']['nickname'], 'avatar' => tomedia($chat['fromuser']['avatar']), 'type' => $log['type'], 'toopenid' => $chat['toopenid'], 'addtime' => TIMESTAMP, 'content' => $log['content']);
    $insert_id = pdo_insert('tiny_wmall_kefu_chat_log', $insert);
    if (!empty($insert_id)) {
        if ($chat['fromfans'] == 1) {
            $update = array('kefunotread' => $chat['kefunotread'] + 1, 'notread' => 0, 'fansdel' => 0, 'kefudel' => 0, 'lastcontent' => $log['content'], 'msgtype' => $log['type'], 'lasttime' => TIMESTAMP);
        } else {
            $update = array('notread' => $chat['notread'] + 1, 'kefunotread' => 0, 'fansdel' => 0, 'kefudel' => 0, 'kefulastcontent' => $log['content'], 'kefumsgtype' => $log['type'], 'kefulasttime' => TIMESTAMP);
        }
        pdo_update('tiny_wmall_kefu_chat', $update, array('uniacid' => $_W['uniacid'], 'id' => $chat['id']));
    }
    $insert['isleft'] = 0;
    ...
    $send = $insert;
    $send['isleft'] = 1;
    $message = array('type' => 'message', 'data' => array('chat' => $send));
    if ($chat['fromfans'] == 1 && $chat['relation'] == 'member2kefu') {
        $user = array('role' => 'kefu', 'token' => $chat['kefuopenid']);
...
{
    global $_W;
    $basics = array('member2clerk' => array('member' => array('订单出餐品了吗'), 'clerk' => array('找我干啥?')), 'member2deliveryer' => array('member' => array('订单什么时候能送到'), 'deliveryer' => array('外卖马上到')), 'member2kefu' => array('member' => array('我想咨询一下'), 'kefu' => array('有什么可以帮您')));
...
function kefu_add_fastreply($user, $content, $relation = 'member2clerk')
{
    ...
    $reply = pdo_get('tiny_wmall_kefu_fastreply', array('uniacid' => $_W['uniacid'], 'relation' => $relation, 'useropenid' => $user['token']));
    if (empty($reply)) {
        $reply = array('uniacid' => $_W['uniacid'], 'relation' => $relation, 'useropenid' => $user['token'], 'content' => iserializer(array()), 'addtime' => TIMESTAMP);
    ...
    $update = array('content' => iserializer($reply['content']));
...
function kefu_get_mychat($user)
{
    ...
    $result = array('chats' => $chats, 'total' => $total, 'pagetotal' => $pagetotal);
    return $result;
}
...
function keft_set_notread_zero($chatOrId, $fromuser)
{
    ...
    if ($fromuser['token'] == $chat['fansopenid']) {
        $update = array('notread' => 0);
    } elseif ($fromuser['token'] == $chat['kefuopenid']) {
        $update = array('kefunotread' => 0);
    }
...
{
    global $_W;
    $tables = array('member' => array('table' => 'tiny_wmall_members', 'field' => 'uid'), 'clerk' => array('table' => 'tiny_wmall_clerk', 'field' => 'id'), 'deliveryer' => array('table' => 'tiny_wmall_deliveryer', 'field' => 'id'));
...
{
    $data = array('member2kefu' => array('text' => '顾客和客服', 'css' => 'label label-success'), 'member2clerk' => array('text' => '顾客和店员', 'css' => 'label label-info'), 'member2deliveryer' => array('text' => '顾客和配送员', 'css' => 'label label-primary'));
    return $data;
}
function kefu_chat_status()
{
    $data = array('1' => array('text' => '正常', 'css' => 'label label-success'), '2' => array('text' => '关闭', 'css' => 'label label-default'));
    return $data;
}
...
function kefu_offline_reply()
{
    ...
    $params = array(':uniacid' => $_W['uniacid'], ':kefuopenid' => $_W['kefu']['user']['token']);
    $chats = pdo_fetchall('select * from ' . tablename('tiny_wmall_kefu_chat') . $condition, $params);
    if (!empty($chats)) {
        $reasons = array('member2kefu' => '客服不在线，暂时无法聊天', 'member2deliveryer' => '配送员不在线，暂时无法聊天', 'member2clerk' => '门店客服不在线，暂时无法聊天');
        foreach ($chats as $chat) {
            $insert = array('uniacid' => $_W['uniacid'], 'chatid' => $chat['id'], 'openid' => $chat['kefuopenid'], 'nickname' => $chat['kefunickname'], 'avatar' => tomedia($chat['kefuavatar']), 'type' => 'system', 'toopenid' => $chat['fansopenid'], 'addtime' => TIMESTAMP, 'content' => $reasons[$chat['relation']]);
            $insert_id = pdo_insert('tiny_wmall_kefu_chat_log', $insert);
            if (!empty($insert_id)) {
                $update = array('status' => 2, 'reason' => $reasons[$chat['relation']]);
                pdo_update('tiny_wmall_kefu_chat', $update, array('uniacid' => $_W['uniacid'], 'id' => $chat['id']));
            }
            $send = $insert;
            $message = array('type' => 'message', 'data' => array('chat' => $send));
...
{
    global $_W, $_GPC;
    $service = array('uid' => '107', 'avatar' => 'http://tp1.sinaimg.cn/1571889140/180/40030060651/1', 'nickname' => '客服107');
    return $service;
}
function kefu_get_chat_records($filter = array())
{
    ...
    $params = array(':uniacid' => $_W['uniacid'], ':from_uid' => $filter['from_uid'], ':from_uid_type' => $filter['from_uid_type'], ':to_uid' => $filter['to_uid'], ':to_uid_type' => $filter['to_uid_type'], ':starttime' => $starttime, ':endtime' => $endtime);
    ...
    if (!empty($records)) {
        ...
        foreach ($records as $record) {
            ...
            $data[] = array('id' => $record['from_uid'], 'avatar' => $mine ? $from_user['avatar'] : $to_user['avatar'], 'content' => $record['msg_type'] == 3 ? iunserializer($record['content']) : $record['content'], 'mine' => $mine, 'timestamp' => $record['createtime'] * 1000, 'type' => 'kefu', 'username' => $mine ? $from_user['nickname'] : $to_user['nickname'], 'msg_type' => $record['msg_type']);
        }
    }
    return $data;
}
function kefu_service_chat_records_fetchall($from_uid, $from_uid_type)
{
    ...
    $params = array(':uniacid' => $_W['uniacid'], ':from_uid' => $from_uid, ':from_uid_type' => $from_uid_type, ':starttime' => $starttime, ':endtime' => $endtime);
    $records = pdo_fetchall('select * from ' . tablename('tiny_wmall_chat_record') . $condition . ' order by createtime asc ', $params);
    $chatlog = array();
    if (!empty($records)) {
        ...
        foreach ($records as $record) {
            ...
            $chatlog[$key][] = array('id' => $record['from_uid'], 'avatar' => $mine ? $kefu['avatar'] : $members[$record['from_uid']]['avatar'], 'content' => $content, 'mine' => $mine, 'timestamp' => $record['createtime'] * 1000, 'type' => 'kefu', 'username' => $mine ? $kefu['nickname'] : $members[$record['from_uid']]['nickname'], 'msg_type' => $record['msg_type']);
        }
    }
    $history = array();
    if (!empty($chatlog)) {
    ...
    $data = array('chatlog' => $chatlog, 'history' => $history);
    return $data;
}
...
function kefu_get_order($orderId)
{
    ...
    $params = array(':uniacid' => $_W['uniacid'], ':id' => $orderId);
...
function kefu_save_record($mine, $from_uid_type, $to, $to_uid_type)
{
    ...
    $insert = array('uniacid' => $_W['uniacid'], 'from_uid' => intval($mine['id']), 'from_uid_type' => $from_uid_type, 'to_uid' => intval($to['id']), 'to_uid_type' => $to_uid_type, 'content' => $msg_type == 3 ? iserializer($mine['content']) : trim($mine['content']), 'msg_type' => $msg_type, 'createtime' => time(), 'read' => 0);
