...
if ($op == 'chat') {
    ...
    $chatlog = array('logs' => array(), 'min' => 0);
    if (!empty($chat)) {
        $chatlog = kefu_get_chat_log($chat['id'], $kefu);
        keft_set_notread_zero($chat, $kefu);
    }
    $result = array('relation' => $relation, 'chatlog' => $chatlog, 'chat' => $chat, 'kefu' => $kefu, 'fans' => $fans, 'chats' => $chats, 'reply' => kefu_get_fastreply($kefu, $relation), 'order' => kefu_get_order($orderid));
    if ($_W['ispost']) {
    ...
    $update = array('kefu_status' => intval($_GPC['kefu_status']));
