...
if ($op == 'index') {
    ...
    if (empty($_W['member'])) {
        $openid = '1234';
        $fans = array('token' => $openid, 'nickname' => '游客', 'avatar' => '');
    } else {
        $fans = $_W['member'];
        $openid = $_W['member']['token'];
    }
    $toopenid = 'wasd';
    $cservice = array('token' => $toopenid, 'nickname' => '客服', 'avatar' => 'http://tp1.sinaimg.cn/1571889140/180/40030060651/1');
    if (empty($cservice)) {
    ...
    if (empty($chat)) {
        $chatData = array('uniacid' => $_W['uniacid'], 'fansopenid' => $openid, 'fansavatar' => $fans['avatar'], 'fansnickname' => $fans['nickname'], 'kefuopenid' => $cservice['token'], 'kefuavatar' => tomedia($cservice['avatar']), 'kefunickname' => $cservice['nickname']);
    ...
    $cservice = array('token' => 'wasd', 'nickname' => '客服', 'avatar' => 'http://tp1.sinaimg.cn/1571889140/180/40030060651/1');
    //检测是否在工作时间
    ...
    $insert = array('uniacid' => $_W['uniacid'], 'chat_id' => $chat['id'], 'openid' => $chat['fansopenid'], 'nickname' => $chat['fansnickname'], 'avatar' => $chat['fansavatar'], 'type' => $type, 'toopenid' => trim($_GPC['toopenid']), 'addtime' => TIMESTAMP, 'content' => $content);
    $insert_id = pdo_insert('tiny_wmall_kefu_chat_log', $insert);
    if (!empty($insert_id)) {
        $update = array('kefunotread' => $chat['kefunotread'] + 1, 'fansdel' => 0, 'kefudel' => 0, 'lastcontent' => $content, 'msgtype' => $type, 'lasttime' => TIMESTAMP);
