...
/*淘 宝 柠 檬 鱼 科 技 https://shop486845690.taobao.com*/
	...
	if($can_get_total > 0) {
		$type_select_total = pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall_wheel_record') . ' where uniacid = :uniacid and uid = :uid and activity_id = :activity_id and type = :type ', array(':uniacid' => $_W['uniacid'], ':uid' => $_W['member']['uid'], ':activity_id' => $wheel['id'], ':type' => $type));
	...
	if($selected == 1) {
		...
		if($award_type == 'credit1' || $award_type == 'credit2') {
			...
			$note = $award_type == 'credit1' ? "恭喜您！获奖啦！奖品：获得{$award_value['value']}积分，已存入您的积分账户" : "恭喜您！获奖啦！奖品：获得余额{$award_value['value']}{$_W['Lang']['dollarSignCn']}，已加入您的余额账户";
			mload()->model('member');
			$log = array(
				$_W['member']['uid'],
				"幸运抽奖{$note}",
			...
			$note = "恭喜您！获奖啦！奖品：总计{$total_discount}{$_W['Lang']['dollarSignCn']}红包，已发放至您的账户中，快去使用吧";
			...
			if($selected == 1) {
				...
				$note = "恭喜您！获奖啦！奖品：本单免单，奖品价值{$order['final_fee']}{$_W['Lang']['dollarSignCn']}，订单金额将会返还到您的账户余额中";
				mload()->model('member');
				$log = array(
					$_W['member']['uid'],
					"幸运抽奖{$note}",
	...
	if($backstatus > 0 && $back > 0) {
		if(($wheel_params['backlimit'] == 1 && $selected == 0) || $wheel_params['backlimit'] == 0) {
			...
			$back_type_cn = $back_type == 'credit1' ? "{$back}积分" : "余额{$back}{$_W['Lang']['dollarSignCn']}";
			$log = array(
				$_W['member']['uid'],
