...
if (!empty($_POST)) {
    ...
    if ($payment_from == "plugincenter") {
        $config_payment = get_global_config_1("plugincenter.pay_type");
    } else {
    ...
    if ($sign == $_POST["sign"] || $rsaverify) {
        ...
        if (!empty($log)) {
            ...
            if (empty($log["tag"]["dollarrate"])) {
                $log["tag"]["dollarrate"] = 1;
            }
            if ($log["status"] == "0" && ($_POST["total_fee"] / $log["tag"]["dollarrate"] == $log["card_fee"] || $_POST["total_amount"] / $log["tag"]["dollarrate"] == $log["card_fee"])) {
