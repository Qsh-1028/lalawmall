...
function store_update_yucunjin($sid, $fee, $trade_type, $extra, $remark = "")
{
    ...
    $log = array("uniacid" => $_W["uniacid"], "agentid" => $account["agentid"], "sid" => $sid, "trade_type" => $trade_type, "extra" => $extra, "fee" => $fee, "yucunjin" => $now_yucunjin, "addtime" => TIMESTAMP, "hash" => $hash, "remark" => $remark);
...
{
    $data = array("1" => array("css" => "label label-success", "text" => "订单扣除"), "3" => array("css" => "label label-default", "text" => "其他变动"));
    return $data;
}
...
function store_notice_stat($clerk_id = 0)
{
    ...
    $notices = pdo_fetchall("SELECT id FROM " . tablename("tiny_wmall_notice") . " WHERE uniacid = :uniacid and status = 1 AND type = :type AND id > :id", array(":uniacid" => $_W["uniacid"], ":type" => "store", ":id" => $new_id));
    if (!empty($notices)) {
        foreach ($notices as &$notice) {
            $insert = array("type" => "store", "uid" => $clerk_id, "notice_id" => $notice["id"], "is_new" => 1);
...
{
    $data = array("1" => array("text" => "已创建未执行", "css" => "label label-danger"), "2" => array("text" => "执行中", "css" => "label label-success"), "3" => array("text" => "执行结束", "css" => "label label-default"));
    return $data;
}
function store_settle_notice($sid, $type = "clerk", $note = "")
{
    ...
    if ($type == "clerk") {
        ...
        $remark = array("审核时间: " . date("Y-m-d H: i", time()), $note);
        $remark = implode("\n", $remark);
        $send = array("first" => array("value" => $tips, "color" => "#ff510"), "keyword1" => array("value" => $store["title"], "color" => "#ff510"), "keyword2" => array("value" => $store["manager"]["mobile"], "color" => "#ff510"), "keyword3" => array("value" => date("Y-m-d H: i", $store["addtime"]), "color" => "#ff510"), "remark" => array("value" => $remark, "color" => "#ff510"));
        if ($store["status"] == 1) {
