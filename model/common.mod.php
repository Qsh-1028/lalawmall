...
if(!function_exists('get_system_config')) {
	function get_system_config($key = '', $uniacid = -1) {
		...
		$_W['ilang'] = !empty($sysset['iglobal']['lang']) ? $sysset['iglobal']['lang'] : 'zh-cn';
		$_W['DollarType'] = !empty($sysset['iglobal']['dollar']) ? $sysset['iglobal']['dollar'] : 'zh-cn';
		$_W['MapType'] = !empty($sysset['iglobal']['map_type']) ? $sysset['iglobal']['map_type'] : 'gaode';
		$_W['dollarrate'] = !empty($sysset['iglobal']['dollarrate']) ? $sysset['iglobal']['dollarrate'] : 1;
		if(empty($key)) {
			return $sysset;
		}
		$keys = explode('.', $key);
...
function get_available_payment($order_type = '', $sid = 0, $all = false, $orderType = 1) {
	...
	if(empty($order_type)) {
			...
			if(is_h5app()) {
				...
				if(is_vue()) {
					if(!is_weixin()) {
						...
						if(is_glala()) {
							$payment = $_W['we7_wmall']['config']['payment']['app'];
						}
					}
		...
		if($order_type == 'takeout') {
			...
			if(!empty($freelunch) && $freelunch['credit_pay_status'] != 1) {
				$index = array_search('credit', $payment);
				if($index !== false) {
					unset($payment[$index]);
				}
...
if(!function_exists('get_plugin_config')) {
	function get_plugin_config($key = '', $force = false) {
		global $_W;
		$_W['uniacid'] = intval($_W['uniacid']);
		$_W['agentid'] = intval($_W['agentid']);
		$name = "pluginset:{$_W['uniacid']}:{$_W['agentid']}";
		if(!empty($_W['we7_wmall'][$name]) && empty($force)) {
			$pluginset = $_W['we7_wmall'][$name];
		} else {
...
function set_plugin_config($key, $value) {
	...
	$keys = explode('.', $key);
	$counts = count($keys);
	$pluginset = get_plugin_config('', true);
	...
	$data = pdo_update('tiny_wmall_config', array('pluginset' => iserializer($pluginset)), array('uniacid' => $_W['uniacid']));
	return true;
}
...
function get_config_text($name, $agentid = 0) {
	...
	if(in_array($name, array('takeout_delivery_time', 'store_delivery_areas', 'delivery_areas'))) {
...
function get_available_plugin() {
	...
	foreach($plugins as $row) {
		...
		if($row['name'] == 'svip') {
			if(!empty($plugin_config['svip']) && $plugin_config['svip']['basic']['status'] != 1) {
				continue;
			}
		}
		$array[] = $row['name'];
...
function get_agent_perm($key = '', $agentid = 0) {
	...
	$agent['plugins'] = array(
		'errander', 'bargain', 'gohome', 'diypage', 'zhunshibao'
	);
	if(!empty($key)) {
		return $agent[$key];
	}
...
function check_mall_status($agentid = 0) {
	...
	if($_W['is_agent']) {
		...
		$tips = !empty($config['tips']) ? $config['tips'] : '亲,平台休息中。。。';
		if($config['status'] == 2) {
			return error(-1, "{$tips}");
		}
...
function get_all_perms($justkey = false) {
	$all_perms = array(
		...
		'statcenter' => array(
			...
			'perms' => array(
				...
				'statcenter.takeoutAgent' => '代理订单统计',
			)
		),
		'merchant' => array(
			'title' => '商户',
		...
		'deliveryer' => array(
			...
			'perms' => array(
				...
				'deliveryer.policy' => '配送策略',
				'deliveryer.location' => '配送员位置日志',
				'deliveryer.getcash' => '提现申请',
				'deliveryer.current' => '账户明细',
				'deliveryer.comment' => '配送评价',
				'deliveryer.storage' => '配送员回收站',
				'deliveryer.cover' => '注册&登录',
				'deliveryer.notice' => '公告列表',
			)
		),
		'clerk' => array(
			'title' => '店员',
	...
	if(check_plugin_perm('agent')) {
		$all_perms['agent'] = array(
			'title' => '区域代理',
			'perms' => array(
				'agent.agent' => '代理列表',
				'agent.initialize' => '数据初始化',
				'agent.getcash' => '提现申请',
				'agent.current' => '账户明细',
				'agent.config' => '设置',
				'agent.cover' => '代理入口',
	...
	if(check_plugin_perm('gohome')) {
		$all_perms['gohome'] = array(
			...
			'perms' => array(
				...
				'tongcheng.basic' => '同城设置',
				'tongcheng.slide' => '同城幻灯片',
				'tongcheng.information' => '同城帖子',
				'tongcheng.comment' => '同城评论列表',
				'haodian.settle' => '好店入驻设置',
				'haodian.slide' => '好店幻灯片',
				'haodian.category' => '好店商户分类',
				'haodian.store' => '好店商户列表',
			)
		);
	}
	if(check_plugin_perm('svip')) {
		$all_perms['svip'] = array(
			'title' => '超级会员',
			'perms' => array(
				'svip.redpacket' => '超级会员专享红包',
				'svip.goods' => '超级会员专享商品',
				'svip.task' => '超级会员任务',
				'svip.credit' => '超级会员奖励金记录',
				'svip.code' => '超级会员兑换码',
				'svip.setmeal' => '套餐列表',
				'svip.order' => '购买记录',
				'svip.config' => '会员设置',
	...
	if(check_plugin_perm('zhunshibao')) {
		$all_perms['zhunshibao'] = array(
			'title' => '准时宝',
			'perms' => array(
				'zhunshibao.config' => '准时宝设置',
			)
		);
	}
	if(check_plugin_perm('dianwoda')) {
		$all_perms['dianwoda'] = array(
			'title' => '点我达',
			'perms' => array(
				'dianwoda.config' => '基本设置',
			)
		);
	}
	if(check_plugin_perm('kefu')) {
		$all_perms['kefu'] = array(
			'title' => '客服系统',
			'perms' => array(
				'kefu.config' => '设置',
				'kefu.kefu' => '客服接待',
	...
	if(empty($areas) || (empty($areas['normal']) && empty($areas['special']))) {
		...
		if(!empty($areas['special'])) {
			...
			foreach($special as $svalue) {
				...
				if((!$crossNight && $now >= $startHour && $now < $endHour) || ($crossNight && ($now >= $startHour || $now <= $endHour))) {
...
function mlog_types($role = '', $value = 0) {
	if($role == 'operator') {
		$role = 'manager';
	} elseif($role == 'merchanter') {
		$role = 'clerker';
	} elseif($role == 'agent_operator') {
		$role = 'agenter';
	}
	$common = array(
		'1000' => array('key' => 1000, 'type' => '订单完成', 'content'=>'订单完成(订单id:%s)'),
		'1002' => array('key' => 1002, 'type' => '订单取消', 'content'=>'订单取消(订单id:%s)'),

		'3000' => array('key' => 3000, 'type' => '添加店员', 'content'=>'添加店员(店员id:%s)，详情：%s'),
		'3002' => array('key' => 3002, 'type' => '删除店员', 'content'=>'删除店员(店员id:%s)，详情：%s'),

		'4000' => array('key' => 4000, 'type' => '添加配送员', 'content'=>'添加配送员(配送员id:%s)，详情：%s'),
		'4002' => array('key' => 4002, 'type' => '删除配送员', 'content'=>'删除配送员(配送员id:%s)，详情：%s'),
	);
	$type_all = array(
		'manager' => array(
			'1001' => array('key' => 1001, 'type' => '订单部分退款', 'content'=>'平台发起订单部分退款(退款id:%s)'),
			'1004' => array('key' => 1004, 'type' => '订单已退款', 'content'=>'平台将订单设为已退款(订单id:%s)'),
			'1005' => array('key' => 1005, 'type' => '订单发起退款', 'content'=>'平台发起订单退款(订单id:%s)'),

			'2000' => array('key' => 2000, 'type' => '添加商户', 'content'=>'平台后台添加商户(商户id:%s)'),
			'2001' => array('key' => 2001, 'type' => '删除商户', 'content'=>'平台后台删除商户(商户id:%s)'),
			'2002' => array('key' => 2002, 'type' => '商户加入回收站', 'content'=>'平台后台将商户加入回收站(商户id:%s)'),
			'2003' => array('key' => 2003, 'type' => '商户入驻审核通过', 'content'=>'商户入驻审核通过(商户id:%s), 备注：%s'),
			'2004' => array('key' => 2004, 'type' => '商户入驻审核不通过', 'content'=>'商户入驻审核不通过(商户id:%s), 备注：%s'),
			'2005' => array('key' => 2005, 'type' => '平台变动商户账户', 'content'=>'后台变动商户账户(记录id:%s), 备注：%s'),
			'2006' => array('key' => 2006, 'type' => '撤销商户提现', 'content'=>'撤销商户提现(记录id:%s), 备注：%s'),
			'2007' => array('key' => 2007, 'type' => '商户提现打款', 'content'=>'商户提现打款(记录id:%s), 结果:%s'),
			'2008' => array('key' => 2008, 'type' => '商户提现设为已处理', 'content'=>'商户提现设为已处理(记录id:%s)'),
			'2010' => array('key' => 2010, 'type' => '平台创建商户活动', 'content'=>'平台创建商户活动，详情：%s'),
			'2011' => array('key' => 2011, 'type' => '更改商户提现账户', 'content'=>'平台更改商户提现账户(商户id:%s)'),
			'2012' => array('key' => 2012, 'type' => '更改商户营业状态', 'content'=>'平台更改商户营业状态(商户id:%s)'),

			'3001' => array('key' => 3001, 'type' => '编辑店员', 'content'=>'平台编辑店员(店员id:%s)'),

			'4001' => array('key' => 4001, 'type' => '编辑配送员', 'content'=>'平台编辑配送员(配送员id:%s)'),
			'4003' => array('key' => 4003, 'type' => '后台变动配送员账户', 'content'=>'后台变动配送员账户(记录id:%s)'),
			'4004' => array('key' => 4004, 'type' => '撤销配送员提现', 'content'=>'撤销配送员提现(记录id:%s), 备注：%s'),
			'4005' => array('key' => 4005, 'type' => '配送员提现打款', 'content'=>'配送员提现打款(记录id:%s), 结果:%s'),
			'4006' => array('key' => 4006, 'type' => '配送员提现设为已处理', 'content'=>'配送员提现设为已处理(记录id:%s)'),
			'4007' => array('key' => 4007, 'type' => '配送员加入回收站', 'content'=>'平台将配送员加入回收站(配送员id:%s)'),
			'4008' => array('key' => 4008, 'type' => '将配送员从回收站中恢复', 'content'=>'平台将配送员从回收站中恢复(配送员id:%s)'),

			'5000' => array('key' => 5000, 'type' => '添加代理', 'content'=>'平台后台添加代理(代理id:%s)'),
			'5001' => array('key' => 5001, 'type' => '删除代理', 'content'=>'平台后台删除代理(代理id:%s)'),
			'5002' => array('key' => 5002, 'type' => '平台变动代理账户', 'content'=>'平台变动代理账户(记录id:%s), 备注：%s'),
			'5003' => array('key' => 5003, 'type' => '撤销代理提现', 'content'=>'撤销代理提现(记录id:%s, 备注：%s)'),
			'5004' => array('key' => 5004, 'type' => '代理提现打款', 'content'=>'代理提现打款(记录id:%s), 结果:%s'),
			'5005' => array('key' => 5005, 'type' => '代理提现设为已处理', 'content'=>'代理提现设为已处理(记录id:%s)'),
			'5007' => array('key' => 5007, 'type' => '更改代理提现账户', 'content'=>'平台更改代理提现账户(代理id:%s)'),

			'6000' => array('key' => 6000, 'type' => '顾客加入黑名单', 'content'=>'顾客加入黑名单(顾客id:%s)'),
			'6001' => array('key' => 6001, 'type' => '删除顾客', 'content'=>'平台后台删除顾客(uid:%s)'),
			'6002' => array('key' => 6002, 'type' => '平台变动顾客账户', 'content'=>'平台变动顾客账户(uid:%s), 详情:%s'),
			'6003' => array('key' => 6003, 'type' => '平台编辑顾客', 'content'=>'平台编辑顾客(uid:%s)'),
			'6004' => array('key' => 6004, 'type' => '平台设置顾客等级', 'content'=>'平台设置顾客等级(uid:%s), 设置等级id:%s'),
			'6005' => array('key' => 6005, 'type' => '平台设置顾客配送会员卡', 'content'=>'平台设置顾客配送会员卡(uid:%s), 详情:%s'),
			'6006' => array('key' => 6006, 'type' => '平台设置顾客超级会员', 'content'=>'平台设置顾客超级会员(uid:%s), 详情:%s'),
			'6007' => array('key' => 6007, 'type' => '顾客移出黑名单', 'content'=>'顾客移出黑名单(顾客id:%s)'),
		),
		'agenter' => array(
			'1001' => array('key' => 1001, 'type' => '订单部分退款', 'content'=>'代理发起订单部分退款(退款id:%s)'),
			'1004' => array('key' => 1004, 'type' => '订单已退款', 'content'=>'代理将订单设为已退款(订单id:%s)'),
			'1005' => array('key' => 1005, 'type' => '订单发起退款', 'content'=>'代理发起订单退款(订单id:%s)'),

			'2000' => array('key' => 2000, 'type' => '添加商户', 'content'=>'代理后台添加商户(商户id:%s)'),
			'2001' => array('key' => 2001, 'type' => '删除商户', 'content'=>'代理后台删除商户(商户id:%s)'),
			'2002' => array('key' => 2002, 'type' => '商户加入回收站', 'content'=>'代理后台将商户加入回收站(商户id:%s)'),
			'2003' => array('key' => 2003, 'type' => '商户入驻审核通过', 'content'=>'代理商户入驻审核通过(商户id:%s), 备注：%s'),
			'2004' => array('key' => 2004, 'type' => '商户入驻审核不通过', 'content'=>'代理商户入驻审核不通过(商户id:%s), 备注：%s'),
			'2011' => array('key' => 2011, 'type' => '更改商户提现账户', 'content'=>'代理更改商户提现账户(商户id:%s)'),
			'2012' => array('key' => 2012, 'type' => '更改商户营业状态', 'content'=>'代理更改商户营业状态(商户id:%s)'),

			'3001' => array('key' => 3001, 'type' => '编辑店员', 'content'=>'代理编辑店员(店员id:%s)'),

			'4001' => array('key' => 4001, 'type' => '编辑配送员', 'content'=>'代理编辑配送员(配送员id:%s)'),
			'4007' => array('key' => 4007, 'type' => '配送员加入回收站', 'content'=>'代理将配送员加入回收站(配送员id:%s)'),
			'4008' => array('key' => 4008, 'type' => '将配送员从回收站中恢复', 'content'=>'代理将配送员从回收站中恢复(配送员id:%s)'),

			'5006' => array('key' => 5006, 'type' => '代理发起提现申请', 'content'=>'代理发起提现申请(记录id:%s)'),
			'5007' => array('key' => 5007, 'type' => '更改代理提现账户', 'content'=>'代理更改提现账户(代理id:%s)'),
		),
		'clerker' => array(
			'1001' => array('key' => 1001, 'type' => '订单部分退款', 'content'=>'商户发起订单部分退款(退款id:%s)'),

			'2009' => array('key' => 2009, 'type' => '商户发起提现申请', 'content'=>'商户发起提现申请(记录id:%s)'),
			'2011' => array('key' => 2011, 'type' => '更改商户提现账户', 'content'=>'商户更改提现账户(商户id:%s)'),
			'2012' => array('key' => 2012, 'type' => '更改商户营业状态', 'content'=>'商户更改营业状态(商户id:%s)'),
		),
	);
	if(!empty($value)) {
		$type = $common[$value];
...
function kefu_statustypes() {
	$types = array('1' => '在线', '2' => '忙碌', '3' => '离线');
	return $types;
}

function to_kefustatus($status = '', $key = 'text') {
	$data = array(
		'1' => array(
			'text' => '在线',
