...
function store_fetch($id, $field = array())
{
    ...
    if ($data["kabao_status"] == 1 && (!check_plugin_perm("kabao") || get_plugin_config("kabao.status") != 1)) {
        $data["kabao_status"] = 0;
    }
    return $data;
}
function store_fetch_activity($sid, $type = array())
{
    ...
    $activity = array("num" => 0, "items" => array(), "labels" => array());
    if (!empty($data)) {
                        ...
                        if ($da["type"] == "newMember") {
                            ...
                            if ($da["type"] == "couponCollect") {
                                ...
                                if ($da["type"] == "couponGrant") {
                                    ...
                                    $activity["labels"][] = array("title" => "返" . $couponGrant["discount"] . $_W["Lang"]["dollarSignCn"] . "券", "class" => "tag tag-danger");
                                } else {
                                    if ($da["type"] == "bargain") {
                                            ...
                                            if ($da["type"] == "selfPickup" || $da["type"] == "selfDelivery") {
                                                ...
                                                if ($da["type"] == "cashGrant") {
                                                    ...
                                                    if ($da["type"] == "svipRedpacket") {
                                                        ...
                                                        $activity["labels"][] = array("title" => (string) $da["data"]["discount"] . $_W["Lang"]["dollarSignCn"] . "无门槛红包", "class" => "tag tag-svip");
                                                    }
                                                }
                                            }
                                        }
    ...
    if (!empty($times)) {
        foreach ($times as $key => &$row) {
            ...
            if ($store["delivery_fee_mode"] == 1) {
                ...
                $row["delivery_price_cn"] = (string) $row["delivery_price"] . $_W["Lang"]["dollarSignCn"] . "配送费";
            } else {
                $row["delivery_price"] = $store["delivery_price"] + $row["fee"];
        ...
        while ($i < $end) {
            if ($store["delivery_fee_mode"] == 1) {
                $store["delivery_price_cn"] = (string) $store["delivery_price"] . $_W["Lang"]["dollarSignCn"] . "配送费";
            } else {
