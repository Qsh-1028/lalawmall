...
function store_all_activity() {
	return array(
		...
		'newMember' => array(
			'title' => 'دۇكان يىڭى ئەزا ',
			'key' => 'newMember',
			'label' => 'label-danger'
		),
		'discount' => array(
			'title' => 'ئېستىمالغا يەتسە ',
			'key' => 'discount',
			'label' => 'label-danger'
		),
		'cashGrant' => array(
...
function store_fetch($id, $field = array()) {
	...
	if(empty($data)) {
		return error(-1, 'دۇكان يوق ياكى ئۆچۈرۈلگەن');
	}
	if($data['status'] == 4) {
		return error(-1, 'دۇكان ئۆچۈرۈلگەن');
	}
	...
	if(isset($data['business_hours'])) {
		...
		if($data['is_in_business_hours'] && !store_is_in_business_hours($data['business_hours'])) {
			...
			if($rest_order_info['nextday'] == 1) {
				$data['rest_reserve_cn'] = "现在预订， 最早明天{$rest_order_info['delivery_time']}اشلاپ يەتكۈزۈلىدۇ";
			} else {
				$data['rest_reserve_cn'] = "现在预订， 最早{$rest_order_info['delivery_time']}اشلاپ يەتكۈزۈلىدۇ";
			}
		}
		$hour = array();
	...
	if(isset($data['data'])) {
		...
		$data['service_titles'] = array('takeout' => 'تاماق زاكاس ', 'tangshi' => '扫码点餐', 'assign' => '排号','reserve' => '预定', 'paybill' => '当面付');
		...
		if(empty($data['data']['cn'])) {
			$data['data']['cn'] = array(
				'box_price' => 'قاچا ھەققى',
				'pack_fee' => 'ئوراش ھەققى'
			);
		}
...
function store_rest_start_delivery_time($store) {
	...
	if(empty($delivery_time)) {
		...
		$delivery_time_cn = "اكاس قىلىندى ەتە{$delivery_time}اشلاپ يەتكۈزۈلىدۇ";
		$nextday = 1;
	} else {
		$delivery_time_cn = "اكاس قىلىندى {$delivery_time}اشلاپ يەتكۈزۈلىدۇ";
	}
	$data = array(
