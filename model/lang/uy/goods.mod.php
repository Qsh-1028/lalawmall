...
function goods_format($goods)
{
    ...
    if (isset($goods["discount_price"]) && !empty($goods["discount_price"])) {
        ...
        if (empty($goods["svip_status"]) && $goods["kabao_status"] == 1 && 0 < $goods["group_id"] && ORDER_TYPE == "takeout") {
...
function goods_avaliable_fetchall($sid, $cid = 0, $ignore_bargain = false)
{
    ...
    foreach ($goods as &$good) {
        ...
        if (1 < $good["unitnum"]) {
            $unitname_cn = empty($good["unitname"]) ? "كىشلىك" : trim($good["unitname"]);
        ...
        if (empty($good["svip_status"]) && 0 < $good["group_id"] && ORDER_TYPE == "takeout") {
