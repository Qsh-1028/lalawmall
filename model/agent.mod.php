...
function agent_update_account($id, $fee, $trade_type, $extra, $remark = '', $order_type = 'takeout') {
	...
	$hash = md5("{$_W['uniacid']}-{$id}-{$trade_type}-{$extra}-{$order_type}");
	if($trade_type == 2 || $trade_type == 3) {
		//其他变动和申请提现
		$hash = md5("{$_W['uniacid']}-{$id}-{$trade_type}-{$fee}" . TIMESTAMP);
	}
	...
	if($type == 'transfers') {
		...
		if($log['channel'] == 'weixin' || $log['channel'] == 'wxapp') {
			...
			$params = array(
				...
				'desc' => "{$agent['title']}" . date('Y-m-d H:i', $log['addtime']) . "提现申请"
			);
			...
			$params = array(
				...
				'remark' => "{$agent['title']}" . date('Y-m-d H:i', $log['addtime']) . "提现申请"
			);
...
function update_store_agent($storeid, $agentid) {
	global $_W;
	$tables = array(
		'tiny_wmall_store_current_log', 'tiny_wmall_store_account', 'tiny_wmall_store_getcash_log', 'tiny_wmall_activity_bargain', 'tiny_wmall_activity_bargain_goods',
	);
...
function update_deliveryer_agent($deliveryerid, $agentid) {
	global $_W;
	$tables = array(
		'tiny_wmall_deliveryer_current_log', 'tiny_wmall_deliveryer_getcash_log', 'tiny_wmall_store_deliveryer'
	);
...
function get_agent_perms($justkey = true, $from = 'app') {
	...
	if($from == 'app') {
		$all_perms['agent.config'] = array(
			'title' => '设置',
			'perms' => array(
				'agent.config.mall' => '基础设置/分享及关注/平台状态/oAuth设置',
				'agent.config.takeout' => '服务范围/订单相关',
				'agent.config.store' => '配送模式/服务费率/商户入驻/其他批量操作',
				'agent.config.deliveryer' => '配送员申请/提成及提现',
			)
		);
		$all_perms['agent.finance'] = array(
			'title' => '财务',
			'perms' => array(
				'agent.finance.getcash' => '提现',
			)
		);
		...
		$all_perms['finance'] = array(
			'title' => '财务',
			'perms' => array(
				'finance.getcash' => '提现/提现账户',
				'finance.account' => '账户明细',
				'finance.order' => '订单入账',
			)
		);
	}
