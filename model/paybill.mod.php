...
function paybill_order_status_notice($orderOrid, $type, $extra = array())
{
    ...
    if (!empty($order["openid"])) {
        ...
        if ($type == "pay") {
            $title = "您的订单已付款";
            $remark = array("门店名称: " . $store["title"], "支付方式: " . $order["pay_type_cn"], "支付时间: " . date("m-d H:i:s", $order["paytime"]), "订单金额: " . $_W["Lang"]["dollarSign"] . $order["total_fee"], "优惠金额: " . $_W["Lang"]["dollarSign"] . $order["discount_fee"], "实付金额: " . $_W["Lang"]["dollarSign"] . $order["final_fee"]);
        }
        if (!empty($extra)) {
        ...
        $params_send = array("title" => $title, "ordersn" => $order["order_sn"], "final_fee" => $order["final_fee"], "pay_type_cn" => $order["pay_type_cn"], "delivery_title" => "******", "status_cn" => "已支付", "remark" => $remark);
...
function paybill_order_clerk_notice($orderOrid, $type, $extra = array())
{
    ...
    if ($type == "pay") {
        ...
        $remark = array("支付方式: " . $order["pay_type_cn"], "支付时间: " . date("m-d H:i:s", $order["paytime"]), "订单金额: " . $_W["Lang"]["dollarSign"] . $order["total_fee"], "优惠金额: " . $_W["Lang"]["dollarSign"] . $order["discount_fee"], "实付金额: " . $_W["Lang"]["dollarSign"] . $order["final_fee"]);
    }
    if (!empty($extra)) {
        $remark = array_merge($remark, $extra);
    }
...
function paybill_order_update_bill($orderOrid)
{
    ...
    if ($account["fee_paybill"]["type"] == 1) {
        ...
        if ($account["fee_paybill"]["type"] == 3) {
            $levels = array($account["fee_paybill"]["level1"]["fee_start"] => $account["fee_paybill"]["level1"], $account["fee_paybill"]["level2"]["fee_start"] => $account["fee_paybill"]["level2"]);
            krsort($levels);
            ...
            $plateform_serve = array("fee_type" => 3, "fee_start" => $fee_start, "fee" => $plateform_serve_fee, "note" => "订单满" . $fee_start . $_W["Lang"]["dollarSignCn"] . "抽成" . $plateform_serve_fee . $_W["Lang"]["dollarSignCn"]);
        } else {
            $plateform_serve_fee = $account["fee_paybill"]["fee"];
            $plateform_serve = array("type" => 2, "fee_rate" => 0, "fee" => $account["fee_paybill"]["fee"], "note" => "每单固定" . $account["fee_paybill"]["fee"] . $_W["Lang"]["dollarSignCn"]);
        }
    }
