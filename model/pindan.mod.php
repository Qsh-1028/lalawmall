...
function pindan_data_init($sid, $pindan_id, $ignore_bargain = false, $extra = array())
{
	...
	if (empty($goods_ids)) {
		$pindan_data = array(
			'mine'             => array('member' => $_W['member']),
			'price'            => 0,
			'box_price'        => 0,
			'total_cart_price' => 0,
			'pindan_id'        => $pindan_id,
			'pindan_status'    => $groups[$pindan_id]['pindan_status']
			);
		return $pindan_data;
	}
	...
	$pindan_data['total_cart_price'] = $pindan_data['cart_price'] = $pindan_data['price'] + $pindan_data['box_price'];
