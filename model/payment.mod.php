...
function alipay_build($params, $alipay = array())
{
    ...
    if ($trade_type == "APP") {
        ...
        $biz_content = array("subject" => $params["title"], "out_trade_no" => $tid, "total_amount" => $params["global_fee"], "product_code" => "QUICK_MSECURITY_PAY", "goods_type" => 1);
        $set["body"] = (string) $_W["uniacid"] . ":" . $alipay["channel"];
        ...
        $set["total_fee"] = $params["global_fee"];
...
function wechat_build($params, $wechat)
{
    ...
    if ($wechat["version"] == 1) {
        ...
        $package["total_fee"] = $params["global_fee"] * 100;
        $package["fee_type"] = "1";
...
function pc_pay_prep($args)
{
    ...
    if (empty($record)) {
        $record = array("uniacid" => $_W["uniacid"], "order_sn" => $order_sn, "order_id" => $id, "order_type" => $type, "fee" => $order["final_fee"], "status" => 0, "addtime" => time());
