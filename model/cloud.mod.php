...
function cloud_w_query_auth($code, $module) 
{
	...
	$content = cloud_w_request("http:///", $params);
	if( is_error($content) ) 
	{
		return $content;
	}
...
function cloud_w_build_wxapp_authorize_params($wxapp_type = "we7_wmall", $uniacid = 0) 
{
	...
	if( $wxapp_type == "we7_wmall" ) {
		$basic = get_plugin_config('wxapp.basic');
		$secret = $basic['secret'];
	}
	if( $wxapp_type == "we7_wmall_deliveryer" ) {
		$deliveryer = get_plugin_config('wxapp.deliveryer');
		$secret = $deliveryer['secret'];
	}
	if( $wxapp_type == "we7_wmall_manager" ) {
		$manager = get_plugin_config('wxapp.manager');
		$secret = $manager['secret'];
	}
	$params = array( "url" => rtrim($_W["siteroot"], "/"), "host" => $_SERVER["HTTP_HOST"], "module" => "we7_wmall", "wxapp_type" => $wxapp_type, "secret" => $secret, "plugins" => cloud_w_plugins(), "method" => "wxapp_authorize_url", "uniacid" => $uniacid, "account" => $account, "wxapp" => array( "siteurl" => trim($_GPC["siteurl"]), "user_desc" => trim($_GPC["user_desc"]), "menutype" => intval($wxapp["menu"]["type"]), "menu" => $menus, "homepage" => $homepage, "extPages" => $extPages, "window" => $window, "navigateToMiniProgramAppIdList" => $miniProgramAppIdList, "use_diy_home" => $wxapp["diy"]["use_diy_home"], "test" => $wxapp["test"], "category" => array( "first_id" => trim($_GPC["first_id"]), "second_id" => trim($_GPC["second_id"]), "third_id" => trim($_GPC["third_id"]), "first_class" => trim($_GPC["first_class"]), "second_class" => trim($_GPC["second_class"]), "third_class" => trim($_GPC["third_class"]) ) ) );
	return $params;
}
function cloud_w_get_wxapp_authorize_url($wxapp_type = "we7_wmall", $uniacid = 0) 
{
	...
	$content = cloud_w_request("http:///weixin/xcx/authurl.html", $params);
	if( is_error($content) ) 
	{
		return $content;
	}
...
function cloud_w_get_wxapp_authorize_info($wxapp_type = "we7_wmall", $uniacid = 0) 
{
	...
	$content = cloud_w_request("http:///weixin/xcx/auth.html", $params);
	if( is_error($content) ) 
	{
		return $content;
	}
	...
	if( empty($result["message"]) ) 
	{
		return error(-1, "小程序Appid与AppSecret不可为空");
	}
	return $result["message"];
}
function cloud_w_wxapp_bind_tester($wechatid, $wxapp_type = "we7_wmall", $uniacid = 0) 
{
	...
	$content = cloud_w_request("http:///weixin/xcx/bind_tester.html", $params);
	if( is_error($content) ) 
	{
		return $content;
	}
...
function cloud_w_wxapp_unbind_tester($wechatid, $wxapp_type = "we7_wmall", $uniacid = 0) 
{
	...
	$params["wechatid"] = $wechatid;
	$content = cloud_w_request("http:///weixin/xcx/unbind_tester.html", $params);
	if( is_error($content) ) 
	{
		return $content;
	}
...
function cloud_w_wxapp_commit($wxapp_type = "we7_wmall", $uniacid = 0) 
{
	...
	$content = cloud_w_request("http:///weixin/xcx/commit.html", $params);
	if( is_error($content) ) 
	{
		return $content;
	}
...
function cloud_w_wxapp_submit_audit($wxapp_type = "we7_wmall", $uniacid = 0) 
{
	...
	$content = cloud_w_request("http:///weixin/xcx/submit.html", $params);
	if( is_error($content) ) 
	{
		return $content;
	}
...
function cloud_w_wxapp_get_category($wxapp_type = "we7_wmall", $uniacid = 0) 
{
	...
	$content = cloud_w_request("http:///weixin/xcx/get_category.html", $params);
	if( is_error($content) ) 
	{
		return $content;
	}
...
function cloud_w_wxapp_get_url($wxapp_type = "we7_wmall", $uniacid = 0) 
{
	...
	$content = cloud_w_request("http:///weixin/xcx/authget_url.html", $params);
	if( is_error($content) ) 
	{
		return $content;
	}
...
function cloud_w_wxapp_undocodeaudit($wxapp_type = "we7_wmall", $uniacid = 0) 
{
	...
	$content = cloud_w_request("http:///weixin/xcx/undocodeaudit.html", $params);
	if( is_error($content) ) 
	{
		return $content;
	}
...
function cloud_w_wxapp_release($wxapp_type = "we7_wmall", $uniacid = 0) 
{
	...
	$content = cloud_w_request("http:///weixin/xcx/audit.html", $params);
	if( is_error($content) ) 
	{
		return $content;
	}
...
function cloud_w_download($path) 
{
	...
	$dat = cloud_w_request("http:///", $pars, $headers, 300);
	if( is_error($dat) ) 
	{
