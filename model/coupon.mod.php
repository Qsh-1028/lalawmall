...
function coupon_before_timeout_notice()
{
    ...
    if (!empty($coupons)) {
        ...
        foreach ($data as $item) {
            $params = array("first" => (string) $item["nickname"] . "，您的账户下有" . $item["num"] . "张优惠券即将过期，总价值" . $item["discount"] . $_W["Lang"]["dollarSignCn"] . "，记得使用哦~", "keyword1" => "账户优惠券", "keyword2" => (string) $item["discount"] . $_W["Lang"]["dollarSignCn"], "keyword3" => date("Y-m-d H:i", TIMESTAMP), "keyword4" => "优惠券即将到期通知", "remark" => implode("\n", array("适用店铺: " . $config["mall"]["title"] . "合作商家", "使用规则: 限有效期内使用", "感谢您对" . $config["mall"]["title"] . "平台的支持与厚爱。点击查看详情>>")));
