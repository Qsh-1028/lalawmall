...
function icheckauth($force = true)
{
    ...
    if (0 < $_W["member"]["uid"]) {
        ...
        $sid = intval($_GPC["sid"]);
        if (0 < $sid && check_plugin_perm("kabao")) {
            pload()->model("kabao");
    ...
    if ($force) {
        ...
        if (defined('IN_WXAPP')) {
            ...
            if (defined('IN_VUE')) {
                $result = array("errno" => 41009, "message" => "请先登录", "sessionid" => $_W["session_id"], "weixin" => !check_plugin_exist("customerApp"), "oauthurl" => imurl("system/common/vuesession/userinfo", array("state" => "we7sid-" . $_W["session_id"], "from" => "vue"), true));
                imessage($result, "", "ajax");
            } else {
                if (is_qianfan()) {
                    if ($_W["ispost"]) {
...
function member_union($unionId, $field = "unionId")
{
    ...
    if (!empty($ids)) {
        foreach ($ids as $id) {
            $log = array("uniacid" => $_W["uniacid"], "fromuid" => $id, "touid" => $update["uid"], "addtime" => TIMESTAMP);
    ...
    $tables = array("ims_tiny_wmall_activity_redpacket_record", "ims_tiny_wmall_activity_coupon_record", "ims_tiny_wmall_address", "ims_tiny_wmall_order", "ims_tiny_wmall_order_comment", "ims_mc_credits_record");
...
function member_credit_update($uid, $credittype, $creditval = 0, $log = array(), $wxtpl_notice = true)
{
    ...
    if ($member["is_sys"] == 1) {
        ...
        if (0 < $creditval || 0 <= $value + $creditval) {
            ...
            $data = array("uniacid" => $_W["uniacid"], "uid" => $uid, "credittype" => $credittype, "num" => $creditval, "createtime" => TIMESTAMP, "operator" => intval($log[0]), "module" => "we7_wmall", "clerk_id" => 0, "store_id" => 0, "remark" => $log[1]);
    ...
    if (!empty($wxtpl_notice) && !is_error($result)) {
        ...
        if ($credittype == "credit1") {
            ...
            $params = array("first" => "您在" . $config["mall"]["title"] . "的账户余额有新的变动", "keyword1" => "余额变动", "keyword2" => (string) $creditval . $_W["Lang"]["dollarSignCn"], "keyword3" => date("Y-m-d H:i", TIMESTAMP), "keyword4" => 0 < $creditval ? "余额充值" : "余额消费", "remark" => implode("\n", array("账户余额:" . $member["credit2"], "备注: " . $log[1])));
        }
...
function member_recharge_status_update($order_id, $type, $params)
{
    ...
    if ($type == "pay") {
        ...
        if (0 < $tag["credit2"]) {
            $log = array($order["uid"], "用户充值" . $tag["credit2"] . $_W["Lang"]["dollarSignCn"]);
            member_credit_update($order["uid"], "credit2", $tag["credit2"], $log);
        }
        if (!empty($tag["grant"])) {
            $array = array("credit1" => "积分", "credit2" => $_W["Lang"]["dollarSignCn"]);
            $log = array($order["uid"], "用户充值" . $tag["credit2"] . $_W["Lang"]["dollarSignCn"] . "赠送" . $tag["grant"]["back"] . $array[$tag["grant"]["type"]]);
...
function member_takeout_address_check($sidOrStore, $idOrAddress)
{
    ...
    if (1 < $store["order_address_limit"] || !empty($store["address_type"])) {
...
function member_push_token($memberOrUid)
{
    ...
    $relation = array("alias" => $config["serial_sn"], "tags" => array($member["token"]));
    $code = md5(iserializer($relation));
    $relation["code"] = $code;
    return $relation;
}
function Jpush_member_send($title, $alert, $extras = array(), $memberOrUid)
{
    global $_W;
    $config = $_W["we7_wmall"]["config"]["app"]["customer"];
    ...
    $audience = array("alias" => array($config["serial_sn"]), "tag" => array($member["token"]));
    ...
    $jpush_ios = array("platform" => "ios", "audience" => $audience, "notification" => array("alert" => $alert, "ios" => array("alert" => $alert, "sound" => "", "badge" => "+1", "extras" => $extras)), "options" => array("apns_production" => 1));
