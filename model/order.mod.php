    ...
    if ($type == "collect") {
        ...
        if ($order["data"]["yinsihao_status"] == 1) {
            ...
            if (is_error($yinsihao)) {
                ...
                $order["mobile"] = (string) $yinsihao["secret_mobile"] . "转" . $yinsihao["extension"];
            }
        }
        $num = 0;
                ...
                if ($li["is_print_all"] == 1) {
                    ...
                    if (!empty($order["goods"]) || $order["order_type"] == 4) {
                        ...
                        if (0 < $order["box_price"]) {
                            $content[] = "餐盒　费:" . $order["box_price"] . $_W["Lang"]["dollarSignCn"];
                        }
                        if (0 < $order["pack_fee"]) {
                            $content[] = "包装　费:" . $order["pack_fee"] . $_W["Lang"]["dollarSignCn"];
                        }
                        if (0 < $order["delivery_fee"]) {
                            $content[] = "配送　费:" . $order["delivery_fee"] . $_W["Lang"]["dollarSignCn"];
                        }
                        if (0 < $order["extra_fee"]) {
                            foreach ($order["data"]["extra_fee"] as $item) {
                                $content[] = (string) $item["name"] . ":" . $item["fee"] . $_W["Lang"]["dollarSignCn"];
                            }
                        }
                        if (3 <= $order["order_type"]) {
                            $content[] = "";
                            $content[] = "合　　计:<L>" . $order["total_fee"] . $_W["Lang"]["dollarSignCn"] . "</L>";
                            $content[] = "";
                            if (0 < $order["discount_fee"]) {
                                $content[] = "线上优惠:<L>-" . $order["discount_fee"] . $_W["Lang"]["dollarSignCn"] . "</L>";
                                $content[] = "";
                                $content[] = "实际支付:<L>" . $order["final_fee"] . $_W["Lang"]["dollarSignCn"] . "</L>";
                                $content[] = "";
                            }
                        } else {
                            $content["total_fee"] = "合　　计:" . $order["total_fee"] . $_W["Lang"]["dollarSignCn"];
                            if (0 < $order["discount_fee"]) {
                                $content[] = "线上优惠:-" . $order["discount_fee"] . $_W["Lang"]["dollarSignCn"];
                                $content["final_fee"] = "实际支付:" . $order["final_fee"] . $_W["Lang"]["dollarSignCn"];
                                if ($li["type"] == "365") {
                                    $content["final_fee"] = "<C>实际支付:" . $order["final_fee"] . $_W["Lang"]["dollarSignCn"] . "</C>";
                                }
                            }
                        }
                        if (!empty($order["data"]["member_choice"])) {
                            $content[] = "**********顾客自选服务**********";
                            foreach ($order["data"]["member_choice"] as $choice) {
                                $content[] = (string) $choice["name"] . ":" . $choice["fee"] . $_W["Lang"]["dollarSignCn"];
                            }
                            $content[] = "********************************";
                        }
                        if (!empty($grant)) {
                        ...
                        if ($order["order_type"] == 1) {
                            $content[] = "餐具数量：" . $order["person_num"] . "套";
                            if ($li["type"] == "365") {
...
{
    $data = array("h5app" => array("css" => "label label-warning", "text" => "顾客APP下单", "color" => ""), "wxapp" => array("css" => "label label-info", "text" => "小程序下单", "color" => ""), "wap" => array("css" => "label label-success", "text" => "公众号下单", "color" => ""), "qianfan" => array("css" => "label label-success", "text" => "千帆APP", "color" => ""), "majia" => array("css" => "label label-success", "text" => "马甲APP", "color" => ""));
    if (!empty($channel)) {
        $data = $data[$channel];
        if (empty($all)) {
            $data = $data["text"];
...
function order_status_notice($id, $status, $note = "")
{
    ...
    if (!empty($order["openid"])) {
        ...
        if ($order_channel == "wap") {
            ...
            if ($config_wxapp_basic["tpl_consumer_url"] == "wxapp" && $type != "communicate") {
                $miniprogram = array("appid" => $config_wxapp_basic["key"], "pagepath" => "pages/order/index");
            }
            $params_send = array("title" => $title, "ordersn" => $order["ordersn"], "final_fee" => $order["final_fee"], "pay_type_cn" => $order["pay_type_cn"], "delivery_title" => $order["delivery_title"], "status_cn" => $order["status_cn"], "remark" => $remark);
            ...
            $send = array("keyword1" => array("value" => "#" . $order["serial_sn"], "color" => "#ff510"), "keyword2" => array("value" => (string) $order["order_type_cn"], "color" => "#ff510"), "keyword3" => array("value" => $order["status_cn"], "color" => "#ff510"), "keyword4" => array("value" => $order["username"], "color" => "#ff510"), "keyword5" => array("value" => $order["mobile"], "color" => "#ff510"), "keyword6" => array("value" => date("Y-m-d H:i"), "color" => "#ff510"), "keyword7" => array("value" => $order["final_fee"], "color" => "#ff510"), "keyword8" => array("value" => $order["ordersn"], "color" => "#ff510"));
    ...
    if ($order["order_channel"] == "h5app") {
        ...
        if ($order["order_channel"] == "app" && $order["pay_type"] == "majia") {
            $data = $router[$type];
            if (empty($data)) {
                return true;
            }
            ...
            $params = array("user_id" => $member["uid_majia"], "type" => "remind", "content" => array("tag" => "订单状态变更提醒", "title" => $data["title"], "subtitle" => $data["title"], "link" => ivurl("pages/order/detail", array("id" => $order["id"]), true), "extra_info" => array(array("key" => "消息内容", "val" => $data["content"]))), "is_push" => 1);
...
function order_clerk_notice($id, $type, $note = "", $extra = array())
{
    ...
    if ($type == "place_order") {
        ...
        $title = "您的店铺有新的外卖订单,订单号为 #" . $order["serial_sn"] . " ,订单金额:" . $order["final_fee"] . $_W["Lang"]["dollarSignCn"] . ",请请尽快处理";
                    ...
                    if ($type == "store_order_pay") {
                        ...
                        if ($type == "reserve_order_pay") {
                            ...
                            if ($type == "store_order_jiacai") {
                                $title = (string) $order["table"]["title"] . "客人加菜了, 订单号为" . $order["ordersn"] . ", 请尽快处理";
                                ...
                                unset($goods_temp);
                                $goods = implode("", $goods);
                                $remark = array("门店名称： " . $store["title"], "桌　　号：" . $order["table"]["title"], "商品信息： " . $goods, "加菜时间： " . date("Y-m-d H:i"), "总金　额： " . $order["final_fee"], "支付状态： " . $order["pay_type_cn"]);
                            } else {
                                if ($type == "cancel") {
                                    $title = "订单号为 #" . $order["serial_sn"] . "的订单已被取消";
                                    $remark = array("门店名称: " . $store["title"], "下单时间: " . date("Y-m-d H:i", $order["addtime"]));
                                }
                            }
                        }
    ...
    if ($channel_notice == "wechat") {
        ...
        $data = array($title, $store["title"], "#" . $order["serial_cn"], $order["order_type_cn"], date("Y-m-d H:i", $order["addtime"]), $order["note"], (string) $_W["Lang"]["dollarSign"] . $order["final_fee"]);
    ...
    foreach ($clerks as $clerk) {
        ...
        if ($clerk["extra"]["accept_wechat_notice"] == 1) {
            if ($channel_notice == "wechat") {
                ...
                if (in_array($type, array("place_order", "store_order_place", "cancel"))) {
    ...
    if (in_array($type, array("place_order", "remind", "store_order_place", "cancel"))) {
        ...
        $data = Jpush_clerk_send($msg, $title, array("voice_text" => $title, "url" => $url, "notify_type" => $type, "id" => $order["id"]), $audience);
    }
    return true;
}
function order_deliveryer_notice($id, $type, $deliveryer_id = 0, $note = "")
{
    ...
    if ($type == "new_delivery") {
        ...
        if ($show_mobile == 1) {
            ...
            $remark = array("门店名称: " . $store["title"], "下单时间: " . date("Y-m-d H:i", $order["addtime"]), "总金　额: " . $order["final_fee"], "支付状态: " . $order["pay_type_cn"], "送货地址: " . $order["address"], "订单类型: " . ($store["delivery_mode"] == 1 ? "店内配送单" : "平台配送单"));
        }
        if (!empty($note)) {
            $remark[] = $note;
        }
        ...
        if ($type == "delivery_wait") {
            ...
            if ($show_mobile == 1) {
                ...
                $remark = array("门店名称: " . $store["title"], "下单时间: " . date("Y-m-d H:i", $order["addtime"]), "送货地址: " . $order["address"], "订单类型: " . ($store["delivery_mode"] == 1 ? "店内配送单" : "平台配送单"));
            }
            if (!empty($note)) {
                $remark[] = $note;
            }
            ...
            if ($type == "cancel") {
                ...
                if ($type == "direct_transfer") {
                    ...
                    if ($show_mobile == 1) {
                        ...
                        $remark = array("门店名称: " . $store["title"], "下单时间: " . date("Y-m-d H:i", $order["addtime"]), "转单时间: " . date("Y-m-d H:i", TIMESTAMP), "送货地址: " . $order["address"], "订单类型: " . ($store["delivery_mode"] == 1 ? "店内配送单" : "平台配送单"));
                    }
                    $remark = implode("\n", $remark);
                    ...
                    if ($type == "direct_transfer_refuse") {
                        ...
                        if ($type == "remind") {
                            ...
                            if ($show_mobile == 1) {
                                ...
                                $remark = array("门店名称: " . $store["title"], "下单时间: " . date("Y-m-d H:i", $order["addtime"]), "送货地址: " . $order["address"]);
                            }
                            $remark = implode("\n", $remark);
                            $url = imurl("delivery/order/takeout/detail", array("id" => $order["id"]), true);
                        }
...
{
    $pay_types = array("" => array("css" => "label label-info", "text" => "未支付"), "alipay" => array("css" => "label label-info", "text" => "支付宝"), "wechat" => array("css" => "label label-success", "text" => "微信支付"), "yimafu" => array("css" => "label label-success", "text" => "一码付"), "credit" => array("css" => "label label-warning", "text" => "余额支付"), "delivery" => array("css" => "label label-primary", "text" => "货到付款"), "cash" => array("css" => "label label-primary", "text" => "现金支付"), "qianfan" => array("css" => "label label-primary", "text" => "千帆APP支付"), "majia" => array("css" => "label label-primary", "text" => "马甲APP支付"), "peerpay" => array("css" => "label label-primary", "text" => "找人代付"), "eleme" => array("css" => "label label-primary", "text" => "饿了么支付"), "maituan" => array("css" => "label label-primary", "text" => "美团支付"), "finishMeal" => array("css" => "label label-primary", "text" => "餐后付款"));
    return $pay_types;
}
function order_check_member_cart($sid)
{
    ...
    foreach ($cart["data"] as $goods_id => $cart_item) {
        ...
        if (!$goods_info[$goods_id]["is_options"]) {
            ...
            if (0 < $option_item["discount_num"] && $option_item["bargain_type"] == "bargain") {
...
function order_insert_refund_log($id, $refund_id, $type, $note = "")
{
    ...
    if (in_array($type, array("apply", "handle"))) {
        ...
        if ($refund["type"] == "goods") {
            $titles = array("apply" => (string) $_W["role_cn"] . "发起部分退款", "handle" => (string) $_W["we7_wmall"]["config"]["mall"]["title"] . "接受部分退款申请");
            $title = $titles[$type];
        }
    }
...
function order_begin_payrefund($refund_id)
{
    ...
    if ($refund["pay_type"] == "credit") {
        if (0 < $refund["uid"]) {
            $log = array($refund["uid"], "外送模块订单退款, 订单号:" . $refund["order_sn"] . ", 退款金额:" . $refund["fee"] . $_W["Lang"]["dollarSignCn"], "we7_wmall");
            mload()->model('member');
    ...
    if ($refund["pay_type"] == "majia") {
        ...
        $refund_update = array("status" => 3, "account" => "支付用户的马甲APP账户余额", "channel" => "ORIGINAL", "handle_time" => TIMESTAMP, "success_time" => TIMESTAMP);
        pdo_update('tiny_wmall_order_refund', $refund_update, array("uniacid" => $_W["uniacid"], "id" => $refund["id"]));
        set_order_refund_status($refund["order_id"], 3);
        order_insert_refund_log($refund["order_id"], $refund["id"], "handle");
        order_insert_refund_log($refund["order_id"], $refund["id"], "success");
...
function order_status_update($id, $type, $extra = array())
{
    ...
    $store = pdo_get("tiny_wmall_store", array("uniacid" => $_W["uniacid"], "id" => $order["sid"]), array("id", "delivery_mode", "auto_handel_order", "auto_print_order", "auto_notice_deliveryer", "kabao_status", "data"));
    ...
    if ($type == "notify_deliveryer_collect") {
        ...
        if ($extra["channel"] == "re_notify_deliveryer_collect") {
            ...
            if (in_array($extra["role"], array("dada", "uupaotui", "shansong", "dianwoda"))) {
                $third_partys = array("dada" => "达达", "uupaotui" => "UU跑腿", "shansong" => "闪送", "dianwoda" => "点我达");
                order_insert_status_log($order["id"], "third_party_cancel_order", (string) $third_partys[$extra["role"]] . "配送取消了订单, 平台正在重新分配配送员");
            }
        } else {
    ...
    if ($type == "cancel") {
        ...
        if (is_open_order($order) || !$order["is_pay"] || $order["final_fee"] <= 0 || $order["is_pay"] == 1 && ($order["pay_type"] == "delivery" || $order["pay_type"] == "cash")) {
            ...
            $note = array("取消原因: " . $extra["note"], "退款金额: " . $refund_fee . $_W["Lang"]["dollarSignCn"], "已付款项会在1-3工作日内返回您的账号");
            ...
            if (!in_array($_W["role"], array("merchanter", "clerker")) && 1 < $order["status"] && $order["is_pay"] == 1) {
                order_clerk_notice($order["id"], "cancel", "取消原因: " . $extra["note"]);
            }
        }
    ...
    if ($type == "end") {
        ...
        if (2 < $order["order_type"] && $order["is_pay"] == 0) {
        ...
        if ($config_takeout["delivery_success_verify"] == 1 && $_W["role"] == "deliveryer") {
            ...
            if (!empty($_deliveryer["location_y"]) && !empty($_deliveryer["location_x"]) && !empty($order["location_y"]) && !empty($order["location_x"])) {
                ...
                if ($config_takeout["delivery_success_verify_distance"] < $distance) {
        ...
        if ($order["delivery_type"] == 2) {
            ...
            if ($config_takeout["deliveryPay_update_deliveryerAccount"] == 1 && $order["pay_type"] == "delivery") {
                $note = (string) $order["id"] . "属于货到支付单,您线下收取客户" . $order["final_fee"] . $_W["Lang"]["dollarSignCn"] . ",平台从您的账户扣除该费用";
        ...
        if ($order["is_pay"] == 1) {
            ...
            $store_account = store_account($order["sid"], array("fee_yucunjin"));
            $fee_yucunjin = floatval($store_account["fee_yucunjin"]);
            if (0 < $fee_yucunjin) {
                store_update_yucunjin($order["sid"], 0 - $fee_yucunjin, 1, $order["id"], (string) $order["order_type_cn"] . "订单扣除");
            }
            if (0 < $order["agentid"]) {
                mload()->model("agent");
                $remark = "";
        ...
        $member_mall = pdo_get("tiny_wmall_members", array("uniacid" => $_W["uniacid"], "uid" => $order["uid"]));
        if (!empty($member_mall)) {
            $member_update = array("success_num" => $member_mall["success_num"] + 1, "success_price" => round($member_mall["success_price"] + $order["final_fee"], 2), "success_last_time" => TIMESTAMP, "stat_last_day" => date("Ymd"));
            if (!$member_mall["success_first_time"]) {
                $member_update["success_first_time"] = TIMESTAMP;
                $member_update["stat_first_day"] = date("Ymd");
            }
            pdo_update('tiny_wmall_members', $member_update, array("id" => $member_mall["id"]));
            $member_store = pdo_get("tiny_wmall_store_members", array("uniacid" => $_W["uniacid"], "sid" => $order["sid"], "uid" => $order["uid"]));
            if (empty($member_store)) {
                $insert = array("uniacid" => $_W["uniacid"], "sid" => $order["sid"], "uid" => $order["uid"], "openid" => $order["openid"], "success_first_time" => TIMESTAMP, "stat_first_day" => date("Ymd"), "success_last_time" => TIMESTAMP, "stat_last_day" => date("Ymd"), "success_num" => 1, "success_price" => $order["final_fee"]);
                pdo_insert('tiny_wmall_store_members', $insert);
            } else {
                $member_update = array("success_num" => $member_store["success_num"] + 1, "success_price" => round($member_store["success_price"] + $order["final_fee"], 2), "success_last_time" => TIMESTAMP, "stat_last_day" => date("Ymd"));
                pdo_update('tiny_wmall_store_members', $member_update, array("id" => $member_store["id"]));
            }
        }
        ...
        if (check_plugin_perm('kabao') && $store["kabao_status"] == 1) {
            pload()->model("kabao");
            kabao_order_give_credit1($store, $order["uid"], $order["final_fee"]);
        }
    ...
    if ($type == "pay") {
        ...
        $update = array("is_pay" => 1, "pay_type" => "cash", "paytime" => TIMESTAMP, "store_final_fee" => 0 - $order["plateform_serve_fee"]);
        pdo_update('tiny_wmall_order', $update, array("uniacid" => $_W["uniacid"], "id" => $order["id"]));
...
function order_deliveryer_update_status($id, $type, $extra = array())
{
    ...
    if ($type == "direct_transfer") {
        ...
        $transfer_log = array("uniacid" => $_W["uniacid"], "deliveryer_id" => $deliveryer["id"], "order_type" => "takeout", "order_id" => $order["id"], "reason" => "定向转单", "addtime" => TIMESTAMP, "stat_year" => date("Y"), "stat_month" => date("Ym"), "stat_day" => date("Ymd"));
        pdo_insert('tiny_wmall_deliveryer_transfer_log', $transfer_log);
    ...
    if ($type == "direct_transfer_reply") {
        ...
        pdo_update('tiny_wmall_order', $update, array("uniacid" => $_W["uniacid"], "id" => $order["id"]));
        pdo_delete('tiny_wmall_deliveryer_transfer_log', array('uniacid' => $_W["uniacid"], "deliveryer_id" => $order["deliveryer_id"], "order_id" => $order["id"]));
...
function order_calculate_deliveryer_fee($order, $deliveryerOrid = 0)
{
    ...
    $special_deliveryer_takeout = $_W["we7_wmall"]["config"]["takeout"]["special"]["deliveryer"]["fee_takeout"];
    if (!empty($special_deliveryer_takeout) && $special_deliveryer_takeout["status"] == 1) {
        if ($special_deliveryer_takeout["type"] == 1 && 0 < $special_deliveryer_takeout["fee"]) {
            $plateform_deliveryer_fee += floatval($special_deliveryer_takeout["fee"]);
        } else {
            if ($special_deliveryer_takeout["type"] == 2 && 0 < $special_deliveryer_takeout["rate"]) {
                $plateform_deliveryer_fee += $plateform_deliveryer_fee * $special_deliveryer_takeout["rate"] / 100;
            }
        }
        $plateform_deliveryer_fee = round($plateform_deliveryer_fee, 2);
    }
...
function order_update_bill($order_id, $extra_data = array())
{
    ...
    if ($fee_config["type"] == 2) {
        ...
        $plateform_serve = array("fee_type" => 2, "fee_rate" => 0, "fee" => $platform_serve_fee, "note" => "每单固定" . $platform_serve_fee . $_W["Lang"]["dollarSignCn"]);
    } else {
        if ($fee_config["type"] == 3) {
            $plateform_serve_rate = 0;
            $levels = array($fee_config["level1"]["fee_start"] => $fee_config["level1"], $fee_config["level2"]["fee_start"] => $fee_config["level2"]);
            krsort($levels);
            $levels = array_values($levels);
            $fee_start = 0;
            $platform_serve_fee = 0;
            if ($levels[0]["fee_start"] <= $order["final_fee"]) {
                $fee_start = $levels[0]["fee_start"];
                $platform_serve_fee = $levels[0]["fee"];
            } else {
                if ($order["final_fee"] < $levels[0]["fee_start"] && $levels[1]["fee_start"] <= $order["final_fee"]) {
                    $fee_start = $levels[1]["fee_start"];
                    $platform_serve_fee = $levels[1]["fee"];
                }
            }
            $plateform_serve = array("fee_type" => 3, "fee_start" => $fee_start, "fee" => $platform_serve_fee, "note" => "订单满" . $fee_start . $_W["Lang"]["dollarSignCn"] . "抽成" . $platform_serve_fee . $_W["Lang"]["dollarSignCn"]);
        } else {
            ...
            if (!empty($account["fee_goods"])) {
                ...
                if (!empty($special_goods_ids)) {
                    foreach ($special_goods_ids as $goodsid) {
                        ...
                        $special_goods_note .= " + " . $cart_goods[$goodsid]["title"] . "佣金 " . $_W["Lang"]["dollarSign"] . $special_goods_plateform_fee_item;
            ...
            if (!empty($fee_config["items_yes"])) {
                ...
                foreach ($fee_config["items_yes"] as $item) {
                    ...
                    $basic += $order[$item];
                    $note["yes"][] = (string) $fee_items["yes"][$item] . " " . $_W["Lang"]["dollarSign"] . $order[$item];
                }
            }
            if (!empty($fee_config["items_no"])) {
                ...
                foreach ($fee_config["items_no"] as $item) {
                    $basic -= $order[$item];
                    $note["no"][] = (string) $fee_items["no"][$item] . " " . $_W["Lang"]["dollarSign"] . $order[$item];
                }
            }
            if ($basic < 0) {
                $basic = 0;
    ...
    if ($order["order_type"] == 1) {
        if ($order["delivery_type"] == 1) {
            ...
            if (!empty($store_bear_deliveryprice)) {
                ...
                $plateform_serve["note"] .= " + 商家额外承担配送费 " . $_W["Lang"]["dollarSign"] . $store_bear_deliveryprice;
            }
        }
    }
    ...
    if (0 < $order["agentid"]) {
        ...
        if ($agent_fee_config["type"] == 2) {
            ...
            $agent_serve = array("fee_type" => 2, "fee_rate" => 0, "fee" => $agent_serve_fee, "note" => "每单固定" . $agent_serve_fee . $_W["Lang"]["dollarSignCn"]);
        } else {
            if ($agent_fee_config["type"] == 3) {
                ...
                $text = "本单代理佣金" . $_W["Lang"]["dollarSign"] . $platform_serve_fee . " x " . $agent_serve_rate . "%";
    ...
    if (0 < $order["zhunshibao_status"]) {
        ...
        $zhunshibao_cn = " + 准时宝收入 " . $_W["Lang"]["dollarSign"] . $order["zhunshibao_price"];
    }
    if ($order["delivery_type"] == 2) {
    ...
    $extra_fee_cn = " + 附加费收入 " . $_W["Lang"]["dollarSign"] . $extra_fee;
    $agent_serve["final"] = "(代理商抽取佣金 " . $_W["Lang"]["dollarSign"] . $platform_serve_fee . " - 平台服务佣金 " . $_W["Lang"]["dollarSign"] . $agent_serve_fee . " - 代理商补贴 " . $_W["Lang"]["dollarSign"] . $order["agent_discount_fee"] . " + 代理商配送费 " . $_W["Lang"]["dollarSign"] . $plateform_delivery_fee . " - 代理商支付给配送员配送费 " . $_W["Lang"]["dollarSign"] . $plateform_deliveryer_fee . $extra_fee_cn . $zhunshibao_cn . ")";
...
{
    global $_W;
    $order = pdo_get("tiny_wmall_order", array("uniacid" => $_W["uniacid"], "id" => $id), array("id", "sid", "price", "box_price", "final_fee", "uid"));
    if (empty($order)) {
        return error(-1, "订单不存在");
    }
    mload()->model('coupon');
    $price = $order["price"] + $order["box_price"];
    $coupon = coupon_grant_available($order["sid"], $price);
...
function goods_total_warning_notice($goodsOrid, $optionOrid, $extra = array())
{
    ...
    if (empty($extra["store"])) {
        $extra["store"] = pdo_get("tiny_wmall_title", array("uniacid" => $_W["uniacid"], "id" => $goods["sid"]), array("title"));
    }
    $tips = "商品" . $goods["title"];
    if (!empty($option)) {
        $tips .= "(规格:" . $option["name"] . "),当前库存" . $extra["total_now"] . $goods["uniname"] . ",达到预警值,快去备货吧！";
    }
    $params = array("first" => $tips, "keyword1" => $extra["store"]["title"], "keyword2" => date("Y-m-d H:i:s"), "keyword3" => "商品" . $goods["title"] . "库存不足");
    $send = sys_wechat_tpl_format($params);
    mload()->model('clerk');
...
function order_delivery_info($store, $predict_index = -1, $condition = array())
{
    ...
    if ($store["address_type"] == 1) {
        ...
        if ($store["delivery_fee_mode"] == 1) {
            ...
            if ($store["delivery_fee_mode"] == 2) {
                ...
                if ($store["delivery_fee_mode"] == 3 && !empty($address) && !is_error($address)) {
                    $area_index = 0;
    ...
    foreach ($delivery_time["times"] as &$time) {
        ...
        $time["total_delivery_price_cn"] = (string) $_W["Lang"]["dollarSign"] . $time["total_delivery_price"] . "配送费";
    }
...
function order_calculate($sidOrStore, $cart, $condition = array())
{
    ...
    $member_choice = array();
    $member_choice_note = array();
    $member_choice_fee = 0;
    if (!empty($store["data"]["member_choice"])) {
        foreach ($store["data"]["member_choice"] as $key => $item) {
            $item["fee"] = floatval($item["fee"]);
            if ($item["status"] == 1 && 0 < $item["fee"]) {
                $item["status"] = 0;
                $member_choice[] = $item;
            }
        }
        if (!empty($condition["member_choice"])) {
            foreach ($condition["member_choice"] as $key => $choice) {
                if ($choice["status"] == 1 && $choice["fee"] == $member_choice[$key]["fee"]) {
                    $member_choice[$key]["status"] = $choice["status"];
                    $member_choice_fee += $member_choice[$key]["fee"];
                    $member_choice_note[] = $choice;
                }
            }
        }
    }
    $extra_fee += $member_choice_fee;
    $buy_svip_fee = 0;
    ...
    $order = array("order_type" => $order_type, "pack_fee" => $store["pack_price"], "price" => $cart["price"], "box_price" => $cart["box_price"], "delivery_fee" => $delivery_info["total_delivery_price"], "extra_fee" => $extra_fee, "extra_fee_detail" => $extra_fee_note, "member_choice" => $member_choice, "member_choice_note" => $member_choice_note, "total_fee" => round($cart["price"] + $cart["box_price"] + $store["pack_price"] + $delivery_info["total_delivery_price"] + $extra_fee, 2), "discount_fee" => $activityed["total"], "activityed" => $activityed, "redpacket" => $redpacket, "coupon" => $coupon, "deliveryTimes" => $delivery_info["delivery_times"], "note" => trim($condition["note"]), "person_num" => trim($condition["person_num"]), "invoiceId" => intval($condition["invoiceId"]), "meal_redpackets" => $meal_redpackets);
    ...
    if (!empty($store["data"]["zhunshibao"]) && $store["data"]["zhunshibao"]["status"] == 1) {
        if ($store["data"]["zhunshibao"]["price_type"] == 1) {
            ...
            if ($store["data"]["zhunshibao"]["price_type"] == 2) {
...
function order_plateform_notice($idOrorder, $type, $extra = array())
{
    ...
    $store = store_fetch($order["sid"], array("title", "id", "push_token"));
    if ($type == "ordernew") {
        $pextra = array("title" => "平台有新的外卖订单", "voice_text" => "店铺" . $store["title"] . "有新的外卖订单,收货地址为" . $order["address"] . ",订单金额" . $order["final_fee"] . $_W["Lang"]["dollarSignCn"], "notify_type" => "ordernew", "url" => ipurl("pages/order/takeout", array("status" => $order["status"]), true), "order_id" => $order["id"], "perm" => "order.takeout");
    }
    mload()->model('jpush');
...
function order_goods_category_limit_check($sid, $cart)
{
    ...
    foreach ($categorys_limit as $limit) {
        ...
        if (0 < $limit["fee"] && $limit["fee"] < $limit["min_fee"]) {
            return error(-1, "分类【" . $limit["title"] . "】最低消费" . $limit["min_fee"] . $_W["Lang"]["dollarSignCn"] . "才能下单");
        }
    }
    return true;
}
...
function order_push_dianwoda($id, $push = false)
{
    ...
    if ($push) {
        ...
        if (!is_error($result)) {
            $data = array("status" => 1, "ordercode" => $result["dwd_order_id"], "skycon" => $result["skycon"], "price" => $result["price"], "distance" => $result["distance"]);
