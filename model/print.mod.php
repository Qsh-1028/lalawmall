...
function print_add_order($printDeviceInfo = array(  ), $order = array(  ))
{
    ...
    if( $printDeviceInfo["type"] == "feie" ) 
    {
        ...
        $postdata = array( "sn" => $printDeviceInfo["deviceno"], "key" => $printDeviceInfo["key"], "printContent" => $printContent, "times" => $printDeviceInfo["times"], "backurl" => (string) $_W["siteroot"] . "addons/we7_wmall/payment/print/notify.php" );
    ...
    else
    {
        if( $printDeviceInfo["type"] == "feie_new" ) 
        {
            ...
            $printContent = implode("<BR>", $printDeviceInfo["content"]);
            $postdata = array( "user" => $user, "stime" => TIMESTAMP, "sig" => $sign, "apiname" => "Open_printMsg", "sn" => $printDeviceInfo["deviceno"], "content" => $printContent, "times" => $printDeviceInfo["times"], "backurl" => (string) $_W["siteroot"] . "addons/we7_wmall/payment/print/notify.php", "callbackurl" => (string) $_W["siteroot"] . "addons/we7_wmall/payment/print/notify.php" );
            if( $printDeviceInfo["language"] == "Uyghur" ) 
            {
                $postdata["language"] = "Uyghur";
            }
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            if( $printDeviceInfo["type"] == "lingdian" ) 
                                            {
                                                $printDeviceInfo["content"] = str_replace(array( "<CB>", "</CB>" ), array( "<middle><dh>", "</dh></middle>" ), $printDeviceInfo["content"]);
                                                $printDeviceInfo["content"] = str_replace(array( "<C>", "</C>" ), array( "</middle>", "</middle>" ), $printDeviceInfo["content"]);
                                                $printDeviceInfo["content"] = str_replace(array( "<L>", "</L>" ), array( "<dh>", "</dh>" ), $printDeviceInfo["content"]);
                                                $printDeviceInfo["content"] = str_replace(array( "<QR>", "</QR>" ), array( "<qrcode>", "</qrcode>" ), $printDeviceInfo["content"]);
                                                $printContent = implode("<br/>", $printDeviceInfo["content"]);
                                                $postdata = array( "secret" => $printDeviceInfo["key"], "print_sn" => $printDeviceInfo["print_no"], "print_num" => $printDeviceInfo["times"], "order_id" => $order["id"], "order_content" => $printContent, "notify_url" => "" );
                                                $posturl = "https://www.yprinter.com/api/Order/addOrder";
                                            }
        ...
        else
        {
            ...
            if( $printDeviceInfo["type"] == "lingdian" ) 
            {
                ...
                if( $result["code"] == 100 ) 
                {
                    ...
                    $result["orderindex"] = $order["ordersn"];
                }
                else
                {
                    $result["responseCode"] = $result["code"];
                    $result["responseMsg"] = $result["message"];
                }
...
function print_2_Uyghur($content)
{
    ...
    {
        return error(-1, (string) $result["code"] . ":" . $result["result"]);
    }
...
{
    return array( 'feie' => array( 'text' => '飞鹅打印机', 'css' => 'label label-success' ), 'feie_new' => array( 'text' => '飞鹅打印机新接口', 'css' => 'label label-success' ), 'feiyin' => array( 'text' => '飞印打印机', 'css' => 'label label-danger' ), 365 => array( 'text' => '365 S1打印机', 'css' => 'label label-warning' ), '365_s2' => array( 'text' => '365 S2打印机', 'css' => 'label label-success' ), 'AiPrint' => array( 'text' => 'AiPrint打印机', 'css' => 'label label-default' ), 'yilianyun' => array( 'text' => '易联云打印机', 'css' => 'label label-primary' ), 'qiyun' => array( 'text' => '启云打印机', 'css' => 'label label-info' ), 'xixun' => array( 'text' => '喜讯打印机', 'css' => 'label label-info' ), 'jinyun' => array( 'text' => '进云打印机', 'css' => 'label label-info' ), 'lingdian' => array( 'text' => '零点云打印机', 'css' => 'label label-info' ) );
}
