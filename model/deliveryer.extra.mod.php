...
function deliveryer_vue_tabs($type = "takeout")
{
    ...
    if ($type == "takeout") {
        ...
        return $tabs = array(array("status" => 3, "status_cn" => "待抢", "num" => $num3), array("status" => 7, "status_cn" => "待取货", "num" => $num7), array("status" => 4, "status_cn" => "配送中", "num" => $num4), array("status" => 5, "status_cn" => "已完成", "num" => 0));
    }
    if ($type == "errander") {
        $config_errander = get_plugin_config("errander");
        ...
        return $tabs = array(array("status" => 1, "status_cn" => "待抢", "num" => $num1), array("status" => 2, "status_cn" => "待取货", "num" => $num2), array("status" => 3, "status_cn" => "配送中", "num" => $num3), array("status" => 4, "status_cn" => "已完成", "num" => 0));
    }
}
function deliveryer_notice_stat($delivery_id = 0)
{
    ...
    if (!empty($notices)) {
        foreach ($notices as &$notice) {
            $insert = array("type" => "delivery", "uid" => $delivery_id, "notice_id" => $notice["id"], "is_new" => 1);
