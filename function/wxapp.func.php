...
function imessage($msg, $redirect = "", $type = "ajax")
{
    ...
    $global = array("system" => array("siteroot" => $_W["siteroot"], "attachurl" => $_W["attachurl"], "cookie_pre" => $_W["config"]["cookie"]["pre"]), "cookie_pre" => $_W["config"]["cookie"]["pre"], "configmall" => $_W["we7_wmall"]["config"]["mall"], "itime" => $_W["ITIMESTAMP"], "DollarType" => $_W["DollarType"], "MapType" => $_W["MapType"]);
    ...
    if (!in_array($_GPC["ctrl"], array("plateform", "manage", "delivery"))) {
        if ($_W["we7_wmall"]["config"]["member"]["force_bind_mobile"] == 1 && !empty($_W["member"]) && empty($_W["member"]["mobile"])) {
            $global["bind_mobile"] = array("show" => 1, "isverifymobile" => intval($_W["we7_wmall"]["config"]["sms"]["verify"]["consumer_register"]));
        }
    ...
    if ($_W["ilang"] == "zhcn2uy") {
        $router = array("wmall/member/address", "wmall/member/mine", "wmall/order/index", "wmall/order/create", "wmall/store/comment");
